...
/** Render a React element to a static ANSI string at the given width.
 *
 *  Drop-in replacement for stock Ink's `renderToString`: produces one
 *  fully-painted frame (no diffing, no cursor moves, no alt-screen) that a
 *  caller can write straight to a stream. Built on `renderToScreen` + the
 *  same full-frame serialization the live renderer uses for a cold repaint,
 *  so styling/width/wrapping match the live surface exactly.
 *
 *  Lines are right-trimmed and joined with '\n'. Hyperlinks (OSC 8) and SGR
 *  styles are preserved; style state is reset at the end of every line so a
 *  trailing trim never leaves dangling escape codes. */
export function renderToString(el: ReactElement, width: number): string {
  ...
  for (let y = 0; y < screen.height; y++) {
    ...
    for (let x = 0; x < screen.width; x++) {
      ...
      const cellStyles = pool.get(cell.styleId)
