  ...
  async copySelectionNoClear(): Promise<string> {
    ...
    if (text) {
      try {
        ...
        if (process.env.INFINITE_TUI_DEBUG_CLIPBOARD ?? process.env.HERMES_TUI_DEBUG_CLIPBOARD) {
          console.error(
            '[clipboard] no path reached the clipboard (headless + no tmux?) — set INFINITE_TUI_FORCE_OSC52=1 to force the escape sequence'
          )
        }
      } catch (err) {
        if (process.env.INFINITE_TUI_DEBUG_CLIPBOARD ?? process.env.HERMES_TUI_DEBUG_CLIPBOARD) {
