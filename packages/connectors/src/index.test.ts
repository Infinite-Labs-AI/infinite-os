...
describe("Meta Ads durable daily history", () => {
  ...
  it("treats an observed action set without a purchase alias as zero while missing actions remains unknown", async () => {
    ...
    const unmappedMissingActions = {
      ...missingActions,
      campaign_id: "c_unmapped_actions_missing",
      objective: "CUSTOM_UNKNOWN",
    ...
    await withMockFetch((url) => {
      ...
      expect(campaigns.get("c_actions_missing")?.conversions).toEqual(expect.arrayContaining([
        expect.objectContaining({
          resultType: "purchase",
          results: 0,
  ...
  it("uses the adset promoted custom event to classify OFFSITE_CONVERSIONS instead of assuming purchase", async () => {
    await withMockFetch((url) => {
      ...
      if (isMetaAdInsightsRequest(url)) return historyResponse({ data: [{
        ...
        actions: [{ action_type: "lead", "7d_click": "1" }],
        action_values: [{ action_type: "offsite_conversion.fb_pixel_purchase", "7d_click": "219" }],
      ...
      expect(payload.conversions).toEqual(expect.arrayContaining([
        expect.objectContaining({ resultType: "lead", results: 1, isPrimary: true, resultsSource: "derived_from_canonical_mapping" }),
        expect.objectContaining({ resultType: "purchase", isPrimary: false, resultsSource: "meta_results_unverified_type" }),
      ]));
      expect(payload.actionsRaw).toMatchObject({
  ...
  function promotedAdsetFetch(
    ...
    return (url: string): Response => {
      ...
      if (isMetaAdsEdgeRequest(url)) return historyResponse({ data: adRows.map((row) => ({
