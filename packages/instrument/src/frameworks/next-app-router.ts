...
import {
  appRelativeConfigOwnership,
  applyManagedNextConfig,
  planNextConfigProxy,
  reverseManagedNextConfig
...
export const nextAppRouterAdapter: FrameworkAdapter = {
  ...
  plan(root, options) {
    ...
    if (!layoutFile) {
      return {
        files: [layoutFilePath, clientComponentPath, analyticsModulePath, ...(proxyPlan?.files ?? [])],
        applyMode: "plan-only",
        instructions: [...(proxyPlan?.instructions ?? [])],
        assumptions: ["Next.js app router placement points are inferred from the app/ tree."],
        blockers: [missingLayoutBlocker, ...(proxyPlan?.blockers ?? [])],
    ...
    if (proxyPlan) {
      blockers.push(...proxyPlan.blockers)
    }
  ...
  uninstall(context) {
    ...
    const configReversal = reverseManagedNextConfig({
      manifest: context.manifest,
      appRootAbsolute: appRoot,
      appRootRelative: context.appRoot,
      dryRun: context.dryRun
    })
    removedFiles.push(...configReversal.removedFiles)
    warnings.push(...configReversal.warnings)
