...
function unconfiguredModelResponse(): ModelResponse {
  return {
    message:
      "Infinite OS chat is ready, but no model client is configured. Run `infinite local setup` or `infinite local model use <provider> <model>` and configure auth before model-backed synthesis."
  };
}
...
async function completeWithCodex(
  ...
  if (!credentials?.token) {
    return { message: "Codex model auth is not configured. Run `codex login` to sign in (from a terminal: `infinite local setup` or `infinite local codex login`)." };
  }
  ...
  const responseBody = JSON.stringify({
    ...
    // every /responses call alongside store:false, so OpenAI's server-side prefix
    // caching reuses the ~15.5k-token stable prefix instead of re-processing it
    // each turn. We pass the chat sessionId (stable across a session's turns).
    // Omitted when absent (non-chat/keyless calls) so those requests stay
  ...
  // `stream: true` is always set in the request body above, so parse the body
  // incrementally regardless of the (absent) content-type header — see
  // responseJson.
  const json = await responseJson(response, "Codex Responses API", request, true);
...
function codexReloginResponse(): ModelResponse {
  return {
    message:
      "Codex model auth expired and could not be refreshed. Run `codex login` to re-authenticate (from a terminal: `infinite local codex login`), then retry."
  };
}
...
function claudeReloginResponse(): ModelResponse {
  return {
    message:
      "Claude model auth expired or is invalid and could not be refreshed. Run `infinite local auth login claude --mode reuse` or `infinite local auth login claude --mode setup-token`, then retry."
  };
}
...
async function completeWithClaude(
  ...
  if (!bearerToken && !apiKey) {
    return {
      message:
        "Claude model auth is not configured. Run `infinite local setup`, `infinite local auth login claude --mode reuse`, or `infinite local auth login claude --mode setup-token`."
    };
  }
  ...
  // `stream: true` is always set in the request body above, so parse the body
  // incrementally regardless of the content-type header — see responseJson.
  const json = await responseJson(response, "Claude Messages API", request, true);
...
function bearerRequest(token: string, body: string): RequestInit {
  return {
    ...
    headers: {
      ...
      "content-type": "application/json",
      // The upstream codex CLI sends `Accept: text/event-stream` on every
      // /responses call (openai/codex codex-api/src/endpoint/responses.rs). We
      // match it for wire fidelity — the backend streams SSE regardless, but
      // this keeps our request byte-shaped like the first-party client.
...
async function responseJson(
  ...
  // Parse the body incrementally whenever the response advertises an event
  // stream OR we explicitly requested one (`stream: true`). The real Codex
  // backend (chatgpt.com/backend-api/codex/responses) returns its SSE with NO
  // content-type header at all (verified: 200 + transfer-encoding: chunked, no
  // content-type — even when Accept: text/event-stream is sent), so gating on
  // content-type alone silently falls back to buffered parsing and replays every
  // delta in one burst. The real codex CLI reads the byte stream as SSE
  // regardless of content-type, so we trust our own request intent — but
  // parseModelStreamBody sniffs the first bytes so a defensive 200+JSON error
  // shape still parses cleanly instead of being fed through the SSE parser.
  if (response.body && (streamRequested || isEventStreamResponse(response))) {
    return parseModelStreamBody(response.body, provider.startsWith("Claude"), streamCallbacks);
  }
  const text = await response.text();
...
// SSE frames begin with one of these field prefixes (`event:`/`data:`/`id:`/
// `retry:`) or a `:` comment line. Anything else on the wire is not an event
// stream — most importantly a JSON object (`{`/`[`/`"`), which some error
// responses return with a 200. We sniff the leading bytes because the Codex
// backend omits content-type, so the header can't be trusted.
function looksLikeEventStream(text: string): boolean {
  const trimmed = text.replace(/^\s+/, "");
  return /^(?:event|data|id|retry):/.test(trimmed) || trimmed.startsWith(":");
}

// Reads a model response body that we requested as a stream. Sniffs the first
// non-empty bytes: if they look like SSE, the body is consumed incrementally so
// message deltas surface progressively (as network chunks arrive); otherwise the
// body is drained and parsed as buffered text (JSON, or SSE-as-text if a
// prefix-less event stream somehow slips through). This is the defensive
// buffered fallback that keeps a 200+JSON shape from being corrupted by the SSE
// parser.
async function parseModelStreamBody(
  ...
  let buffer = "";
  let done = false;
  // Pull just enough leading bytes to classify SSE vs JSON.
  while (!buffer.trim() && !done) {
  ...
  if (looksLikeEventStream(buffer)) {
    return isClaude
      ? parseClaudeEventStreamFromReader(reader, decoder, buffer, done, callbacks)
      : parseCodexEventStreamFromReader(reader, decoder, buffer, done, callbacks);
  }
  // Not SSE — drain the remainder and parse the buffered text. The same decoder
  // is reused so multibyte sequences straddling the sniff boundary decode once.
  while (!done) {
    const chunk = await reader.read();
    done = chunk.done;
    if (chunk.value) {
  ...
  if (buffer.trim().startsWith("event:") || buffer.includes("response.completed")) {
    return isClaude
      ? parseClaudeEventStreamText(buffer, callbacks)
      : parseCodexEventStreamText(buffer, callbacks);
  }
...
async function parseClaudeEventStreamFromReader(
  reader: ReadableStreamDefaultReader<Uint8Array>,
  decoder: TextDecoder,
  seed: string,
  done: boolean,
...
// Incrementally consume an SSE byte stream, invoking `consume` per complete
// event frame (delimited by a blank line) AS chunks arrive — so message deltas
// surface progressively rather than all at once after the full body is read.
// `seed` is the bytes already pulled off `reader` (for content-type-less
// sniffing) and `done` whether that pull already hit end-of-stream; the same
// TextDecoder is threaded through so a multibyte sequence split across the sniff
// boundary decodes exactly once.
async function consumeEventStreamReader(
  reader: ReadableStreamDefaultReader<Uint8Array>,
  decoder: TextDecoder,
  seed: string,
  done: boolean,
  ...
  let buffer = await consumeCompleteEventChunks(seed, consume);
  let finished = done;
  while (!finished) {
