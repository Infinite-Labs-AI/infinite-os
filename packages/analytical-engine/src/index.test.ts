...
describe("analytical engine smoke", () => {
  ...
  describe("Meta Ads conversions/value metrics (Phase 1 §6)", () => {
    ...
    it("routes the conversion-family metrics to the typed conversions view", () => {
      ...
      expect(metricView("stripe_paid_subscribers")).toBe("queryable.vw_stripe_paid_subscribers");
      for (const metric of [
        ...
        expect(metricView(metric)).toBe("queryable.vw_stripe_subscription_lifecycle");
      }
    });
    ...
    it("sums the additive conversion metrics (results, conversion_value, link_clicks, landing_page_views)", () => {
      ...
      expect(aggregateExpression("stripe_current_paid_subscribers", metricColumn("stripe_current_paid_subscribers"))).toBe(
        "sum(stripe_current_paid_subscribers)"
      );
      expect(aggregateExpression("stripe_new_paid_subscribers", metricColumn("stripe_new_paid_subscribers"))).toBe(
        "sum(stripe_new_paid_subscribers)"
      );
    });
  ...
  // ── Cloud-default WRITE seam: createActionHandlers(db, { encryptionKey }) per-workspace custody ──
  // The connect/reconnect write path AND the connect-time connection test must encrypt+decrypt
  // under the caller-supplied per-workspace key, NOT process.env. The env is deliberately set to a
  // WRONG key so that if the option were not threaded, storage would use the wrong key and the
  // connection test would fail to decrypt (connect_source would reject).
  it("createActionHandlers({ encryptionKey }) stores + verifies credentials under the per-workspace key, ignoring a wrong process.env key", async () => {
    const PER_WS_KEY = "engine-per-workspace-key-3333333333333333";
    const WRONG_ENV_KEY = "engine-wrong-env-key-4444444444444444";
    const prev = process.env.GROWTH_OS_ENCRYPTION_KEY;
    process.env.GROWTH_OS_ENCRYPTION_KEY = WRONG_ENV_KEY;
    const stored: { credential_kind?: string; encrypted_payload?: string; oauth_token_id?: string | null } = {};
    vi.stubGlobal(
      "fetch",
      (async () =>
        new Response(JSON.stringify({ results: [] }), {
    ...
    try {
      const handlers = createActionHandlers(db, { encryptionKey: PER_WS_KEY });
      const result = await handlers.connect_source?.(
        {
          provider: "posthog",
          connectionName: "PostHog",
      ...
      // Stored under the PER-WORKSPACE key (override wins), NOT the wrong process.env key.
      expect(isEncryptedCredentialPayload(String(stored.encrypted_payload))).toBe(true);
      const payload = decryptCredentialPayload<Record<string, unknown>>(String(stored.encrypted_payload), PER_WS_KEY);
      expect(payload).toMatchObject({ personalApiKey: "ph-key", projectId: "42" });
      expect(() => decryptCredentialPayload(String(stored.encrypted_payload), WRONG_ENV_KEY)).toThrow();
      // The connect-time connection test decrypted the just-stored credential under the SAME
      // per-workspace key (proves the SyncRequest.encryptionKey seam is threaded end to end) —
      // so it is ok despite the wrong env key.
      expect(result?.data).toMatchObject({ connectionTest: { ok: true, mode: "live", provider: "posthog" } });
    } finally {
      vi.unstubAllGlobals();
      if (prev === undefined) {
  ...
  it("reconnect_source re-stores + verifies the rotated credential under the per-workspace key", async () => {
    const PER_WS_KEY = "engine-per-workspace-key-reconnect-5555555555";
    const WRONG_ENV_KEY = "engine-wrong-env-key-reconnect-6666666666";
    const prev = process.env.GROWTH_OS_ENCRYPTION_KEY;
    process.env.GROWTH_OS_ENCRYPTION_KEY = WRONG_ENV_KEY;
    const stored: { credential_kind?: string; encrypted_payload?: string; oauth_token_id?: string | null } = {};
    vi.stubGlobal(
    ...
    const db: InfiniteOsDb = {
      async query(sql: string, params?: unknown[]) {
        // Capture the freshly-inserted credential (reconnect revokes + inserts a new row).
        if (sql.includes("insert into connection_credentials")) {
          stored.credential_kind = params?.[3] as string;
          stored.encrypted_payload = params?.[4] as string;
          stored.oauth_token_id = (params?.[5] as string | null) ?? null;
        }
        return [];
      },
      async one<T>(sql: string): Promise<T | null> {
        if (sql.includes("select provider from sources")) {
          return { provider: "posthog" } as T;
        }
        if (sql.includes("connection_credentials")) {
          return stored as T;
        }
    ...
    try {
      ...
      const result = await handlers.reconnect_source?.(
        {
          sourceId: "src_posthog_ws",
          credentialPayload: {
            mode: "live",
            projectId: "42",
            personalApiKey: "ph-key-rotated",
            apiHost: "https://posthog.test"
          }
        },
        {
      ...
      const payload = decryptCredentialPayload<Record<string, unknown>>(String(stored.encrypted_payload), PER_WS_KEY);
      expect(payload).toMatchObject({ personalApiKey: "ph-key-rotated" });
      expect(() => decryptCredentialPayload(String(stored.encrypted_payload), WRONG_ENV_KEY)).toThrow();
      expect(result?.data).toMatchObject({ connectionTest: { ok: true, mode: "live", provider: "posthog" } });
    } finally {
      vi.unstubAllGlobals();
  ...
  describe("connect/reconnect connection-test escalation (B2b: transient must not park)", () => {
    const CTX = {
      workspaceId: "workspace",
    ...
    const KEY = "engine-per-workspace-key-b2b-7777777777777";
    const posthogPayload = { mode: "live", projectId: "42", personalApiKey: "ph-key", apiHost: "https://posthog.test" };

    function makeDb(statuses: Array<{ id: string; status: string }>, queries: string[]): InfiniteOsDb {
      const stored: { credential_kind?: string; encrypted_payload?: string; oauth_token_id?: string | null } = {};
      return {
        async query(sql: string, params?: unknown[]) {
          queries.push(sql);
        ...
        async one<T>(sql: string): Promise<T | null> {
          if (sql.includes("select provider from sources")) return { provider: "posthog" } as T;
          if (sql.includes("connection_credentials")) return stored as T;
          return null;
        },
        async close() {},
        async ensureWorkspace() {},
        ...
        async updateSourceStatus(id: string, status: string) {
    ...
    it("connect_source: a TRANSIENT test failure (network blip) does NOT flip the source to error", async () => {
    ...
    it("reconnect_source: a TRANSIENT test failure does NOT flip the source to error", async () => {
      const statuses: Array<{ id: string; status: string }> = [];
      const queries: string[] = [];
      vi.stubGlobal("fetch", (async () => {
        throw new TypeError("fetch failed");
    ...
    it("reconnect_source: a re-check of the STORED credential RE-ASSERTS connected AFTER a successful test (concurrent-terminal-write race guard)", async () => {
      ...
      try {
        ...
        // re-assert ran AFTER the test, so a concurrent sync's terminal write can't strand a
        // successful reconnect at `error`.
        const reAssert = queries.filter(
          (q) => q.includes("set status = 'connected'") && !q.includes("connected_at")
        );
        expect(reAssert.length).toBeGreaterThanOrEqual(1);
      } finally {
        vi.unstubAllGlobals();
      }
    });
  ...
  it("createActionHandlers(db) with no options is backward-compatible (uses the process.env key)", async () => {
    process.env.GROWTH_OS_ENCRYPTION_KEY = "analytical-test-encryption-key";
    const stored: { encrypted_payload?: string } = {};
    vi.stubGlobal(
      "fetch",
      (async () =>
        new Response(JSON.stringify({ results: [] }), {
    ...
    try {
      // One-arg form (all existing callers) — unchanged behavior: stored under the env key.
      const handlers = createActionHandlers(db);
      await handlers.connect_source?.(
        {
          provider: "posthog",
          connectionName: "PostHog",
          credentialPayload: { mode: "live", projectId: "42", personalApiKey: "ph-key", apiHost: "https://posthog.test" }
        },
        { workspaceId: "workspace", authority: "operator", surface: "api", actorId: "operator", sessionId: "session" }
      );
      const payload = decryptCredentialPayload<Record<string, unknown>>(
        String(stored.encrypted_payload),
        "analytical-test-encryption-key"
      );
      expect(payload).toMatchObject({ personalApiKey: "ph-key" });
    } finally {
      vi.unstubAllGlobals();
    }
  });
  ...
  it("breaks down posthog_page_views by operating_system + country over the PostHog audience view (0043)", async () => {
    ...
    const db: InfiniteOsDb = {
      async query<T = Record<string, unknown>>(sql: string, params?: unknown[]): Promise<T[]> {
        queries.push({ sql, params });
        if (sql.includes("from queryable.vw_posthog_site")) {
          return [
            { operating_system: "Mac OS X", country: "United States", posthog_page_views: "2" },
            { operating_system: "Windows", country: "Canada", posthog_page_views: "1" }
          ] as T[];
        }
        return [] as T[];
      },
    ...
    const result = await handlers.run_breakdown_query?.(
      {
        metric: "posthog_page_views",
        groupBy: ["operating_system", "country"]
      },
      {
        workspaceId: "workspace",
        authority: "tool_agent",
    ...
    // The metric routes to the new audience view (no explicit `view` passed — metricView resolves it).
    expect(result?.data).toMatchObject({
      metric: "posthog_page_views",
      view: "queryable.vw_posthog_site",
      groupBy: ["operating_system", "country"],
      rows: [
        { operating_system: "Mac OS X", country: "United States", posthog_page_views: "2" },
        { operating_system: "Windows", country: "Canada", posthog_page_views: "1" }
    ...
    const aggregateQuery = queries.find((entry) => entry.sql.includes("from queryable.vw_posthog_site"));
    expect(aggregateQuery?.sql).toContain("group by operating_system, country");
    expect(aggregateQuery?.sql).toContain("sum(posthog_page_views) as posthog_page_views");
    // Honesty caveats travel on the envelope: pageview-scoped, client-side audience props.
    expect(result?.caveats).toEqual(
      expect.arrayContaining(["source_native_event_counts", "pageview_events_only", "audience_props_client_side_only"])
    );
  });

  it("normalizes the os / device_category audience aliases to the PostHog audience view columns (0043)", async () => {
    const queries: Array<{ sql: string; params?: unknown[] }> = [];
    const db: InfiniteOsDb = {
      async query<T = Record<string, unknown>>(sql: string, params?: unknown[]): Promise<T[]> {
        queries.push({ sql, params });
        if (sql.includes("from queryable.vw_posthog_site")) {
          return [{ operating_system: "iOS", device_type: "mobile", posthog_page_views: "5" }] as T[];
        }
        return [] as T[];
      },
      async one() {
    ...
    const result = await handlers.run_breakdown_query?.(
      {
        // `os` -> operating_system; `device_category` (the GA4 device dim name) -> device_type.
        metric: "posthog_page_views",
        groupBy: ["os", "device_category"]
      },
      {
        workspaceId: "workspace",
        authority: "tool_agent",
    ...
    const aggregateQuery = queries.find((entry) => entry.sql.includes("from queryable.vw_posthog_site"));
    expect(aggregateQuery?.sql).toContain("group by operating_system, device_type");
  });

  it("breaks down site_key_events by event name over the GA4 event view (0061), normalizing the `event` alias", async () => {
    const queries: Array<{ sql: string; params?: unknown[] }> = [];
    const db: InfiniteOsDb = {
      async query<T = Record<string, unknown>>(sql: string, params?: unknown[]): Promise<T[]> {
        queries.push({ sql, params });
        if (sql.includes("from queryable.vw_site_events")) {
          return [
            { event_name: "download_click", site_key_events: "4" },
            { event_name: "purchase", site_key_events: "2" }
          ] as T[];
        }
        return [] as T[];
      },
    ...
    const result = await handlers.run_breakdown_query?.(
      {
        // The per-event key-events split: a property with a second key event (purchase) can
        // finally answer "downloads only". `event` normalizes to event_name.
    ...
    // metricView resolves the new event view; the additive sum reads the aliased column.
    expect(result?.data).toMatchObject({
      metric: "site_key_events",
      view: "queryable.vw_site_events",
      rows: [
        { event_name: "download_click", site_key_events: "4" },
        { event_name: "purchase", site_key_events: "2" }
      ]
    });
    const aggregateQuery = queries.find((entry) => entry.sql.includes("from queryable.vw_site_events"));
    expect(aggregateQuery?.sql).toContain("group by event_name");
    expect(aggregateQuery?.sql).toContain("sum(site_key_events) as site_key_events");
    // key_events may simply not be configured on a property — the caveat must travel.
    expect(result?.caveats).toEqual(
      expect.arrayContaining(["source_native_attribution_only", "key_events_may_be_unconfigured"])
    );
  });

  it("signup_count reads the PURE PostHog lane (0062) — filtered count on vw_posthog_events", async () => {
    const queries: Array<{ sql: string; params?: unknown[] }> = [];
    const db: InfiniteOsDb = {
      async query<T = Record<string, unknown>>(sql: string, params?: unknown[]): Promise<T[]> {
        queries.push({ sql, params });
        if (sql.includes("from queryable.vw_posthog_events")) {
          return [{ landing_page: "/", signup_count: "3" }] as T[];
        }
        return [] as T[];
      },
      async one() {
    ...
    const handlers = createActionHandlers(db);
    const result = await handlers.run_breakdown_query?.(
      { metric: "signup_count", groupBy: ["landing_page"] },
      { workspaceId: "workspace", authority: "tool_agent", surface: "api", actorId: "operator", sessionId: "session" }
    );
    ...
    expect(aggregateQuery?.sql).toContain(
      "coalesce(sum(posthog_event_count) filter (where event_name = 'signup'), 0) as signup_count"
    );
    expect(queries.some((entry) => entry.sql.includes("vw_site_conversion_rate"))).toBe(false);
  });

  it("run_funnel_query rides the SAME-LANE GA4 view (0062) and says visit-to-key-event", async () => {
    const queries: Array<{ sql: string; params?: unknown[] }> = [];
    const db: InfiniteOsDb = {
      async query<T = Record<string, unknown>>(sql: string, params?: unknown[]): Promise<T[]> {
        queries.push({ sql, params });
        if (sql.includes("from queryable.vw_site_conversion_rate")) {
          return [{ site_conversion_rate: "0.25" }] as T[];
        }
        return [] as T[];
      },
      async one() {
    ...
    const handlers = createActionHandlers(db);
    const result = await handlers.run_funnel_query?.(
      {},
      { workspaceId: "workspace", authority: "tool_agent", surface: "api", actorId: "operator", sessionId: "session" }
    );

    // The rate aggregate is the per-grain daily fraction average over the (now GA4-only) view.
    const aggregateQuery = queries.find((entry) => entry.sql.includes("from queryable.vw_site_conversion_rate"));
    expect(aggregateQuery?.sql).toContain("avg(site_conversion_rate) as site_conversion_rate");
    // The funnel names its honest shape: visit → key event (no cross-provider signup claim), and
    // carries the may-be-unconfigured caveat for properties without key events.
    expect(result?.data).toMatchObject({ caveat: "first_phase_funnel_is_visit_to_key_event_only" });
  ...
  // UNBOUNDED-WINDOW GUARD (2026-08-24) — a run_metric_query with no bound on the view's
  // date dimension aggregates ALL-TIME, and nothing in the single-row result says so.
  // Real incident: asked "total revenue over the last 7 days", the agent omitted the
  // occurred_on filters and the ALL-TIME recognized_revenue ($100) was presented as the
  // 7-day number (true 7-day value: $0). The envelope must stamp `unbounded_date_range`
  // whenever the query carries no date bound, so an all-time total can never masquerade
  // as a windowed answer. The guard is purely additive: rows/values are untouched and no
  // default window is injected (that would silently change every existing answer).
  describe("run_metric_query unbounded-window caveat", () => {
    function caveatFakeDb(
      view: string,
      metric: string,
    ...
    it("stamps unbounded_date_range when no date filter is present (the all-time-as-7-day incident)", async () => {
      const queries: Array<{ sql: string; params?: unknown[] }> = [];
      const handlers = createActionHandlers(
        caveatFakeDb("queryable.vw_revenue_by_source", "recognized_revenue", queries)
      );
      const result = await handlers.run_metric_query?.({ metric: "recognized_revenue" }, ctx);
      expect(result?.caveats).toContain("unbounded_date_range");
      // Purely additive: the metric caveats and the rows themselves are untouched.
      expect(result?.caveats).toContain("content_linkage_not_implemented");
      expect((result?.data as { rows: unknown }).rows).toEqual([{ recognized_revenue: "100" }]);
    });

    it("stamps unbounded_date_range when only non-date filters are present", async () => {
      const queries: Array<{ sql: string; params?: unknown[] }> = [];
      const handlers = createActionHandlers(
        caveatFakeDb("queryable.vw_revenue_by_source", "recognized_revenue", queries)
      );
      const result = await handlers.run_metric_query?.(
        {
          metric: "recognized_revenue",
          filters: [{ field: "provider", operator: "equals", value: "stripe" }]
        },
        ctx
      );
      expect(result?.caveats).toContain("unbounded_date_range");
    ...
    it("omits the caveat when occurred_on gte+lte bounds are passed, values unchanged", async () => {
      const queries: Array<{ sql: string; params?: unknown[] }> = [];
      const handlers = createActionHandlers(
        caveatFakeDb("queryable.vw_revenue_by_source", "recognized_revenue", queries)
      );
    ...
    it("treats a single-sided occurred_on bound as bounded", async () => {
      const queries: Array<{ sql: string; params?: unknown[] }> = [];
      const handlers = createActionHandlers(
        caveatFakeDb("queryable.vw_revenue_by_source", "recognized_revenue", queries)
      );
      const result = await handlers.run_metric_query?.(
        {
          metric: "recognized_revenue",
          filters: [{ field: "occurred_on", operator: "gte", value: "2026-08-17" }]
        },
        ctx
      );
      expect(result?.caveats).not.toContain("unbounded_date_range");
    ...
    it("treats a date-alias equals filter as bounded (a single-day window is a window)", async () => {
      const queries: Array<{ sql: string; params?: unknown[] }> = [];
      const handlers = createActionHandlers(
        caveatFakeDb("queryable.vw_revenue_by_source", "recognized_revenue", queries)
      );
      const result = await handlers.run_metric_query?.(
        {
          metric: "recognized_revenue",
          // `date` normalizes to occurred_on; equals pins one day — bounded, not all-time.
          filters: [{ field: "date", operator: "equals", value: "2026-08-20" }]
        },
        ctx
      );
      expect(result?.caveats).not.toContain("unbounded_date_range");
    ...
    it("exempts snapshot-grain metrics — a point-in-time count is not an all-time sum", async () => {
      ...
      const subs = await subsHandlers.run_metric_query?.({ metric: "stripe_current_paid_subscribers" }, ctx);
      expect(subs?.caveats).toContain("current_snapshot");
      expect(subs?.caveats).not.toContain("unbounded_date_range");
    });

    it("exempts stripe_trialing_subscribers — the lifecycle view only holds CURRENTLY-trialing rows", async () => {
      // Migration 0055: the trialing lane counts subs with status='trialing' NOW, keyed by
      // trial_start date. An unbounded sum is therefore the CURRENT trialing count — a
      // point-in-time snapshot, not an all-time total — so the caveat is meaningless here.
      const queries: Array<{ sql: string; params?: unknown[] }> = [];
      const handlers = createActionHandlers(
        caveatFakeDb("queryable.vw_stripe_subscription_lifecycle", "stripe_trialing_subscribers", queries)
      );
      const result = await handlers.run_metric_query?.({ metric: "stripe_trialing_subscribers" }, ctx);
      expect(result?.caveats).toContain("trialing_is_not_paid");
      expect(result?.caveats).not.toContain("unbounded_date_range");
    });

    it("keeps unbounded_date_range within the first 2 caveats on multi-caveat metrics (digest slice)", async () => {
      // toolResultDigest (llm-controller model-client) slices envelope caveats to the first
      // 2 for the curated digest line the model reads. The Meta ratio family carries up to
      // 8 base caveats (frequency: 3), so the unbounded signal must lead the array or it
      // never reaches the model. Engine-side contract: prepend, don't append.
      const queries: Array<{ sql: string; params?: unknown[] }> = [];
      const handlers = createActionHandlers(
        caveatFakeDb("queryable.vw_meta_ads_campaign_daily", "frequency", queries)
      );
      const result = await handlers.run_metric_query?.({ metric: "frequency" }, ctx);
      const caveats = result?.caveats ?? [];
      expect(caveats.slice(0, 2)).toContain("unbounded_date_range");
      // The base metric caveats all survive — only the order changed.
      expect(caveats).toEqual(expect.arrayContaining(caveatsForMetric("frequency")));
    });
  });
  ...
  // today but no invoices in the window — the true answer is a confirmed $0, not a hedge.
  // The engine must coalesce the empty window to 0 AND stamp the typed
  // `zero_confirmed_fresh` marker ONLY when the backing source is connected and fresh;
  // a stale or disconnected source must stay null (a false "$0" is worse than a hedge).
  describe("run_metric_query confirmed-zero window", () => {
    interface ZeroFakeOptions {
      view: string;
      metric: string;
      aggregateRows: Record<string, unknown>[];
      /** sources.status for the connectedSources probe; null = no connected/degraded row. */
      sourceStatus: string | null;
      /** freshness as_of for the provider; null = provider never successfully synced. */
      asOf: string | null;
      provider?: string;
      sourceTables?: string;
    }
    ...
    function zeroFakeDb(
      queries: Array<{ sql: string; params?: unknown[] }>,
      options: ZeroFakeOptions
    ): InfiniteOsDb {
      const provider = options.provider ?? "stripe";
      return {
        async query<T = Record<string, unknown>>(sql: string, params?: unknown[]): Promise<T[]> {
          queries.push({ sql, params });
          if (sql.includes(`from ${options.view}`)) {
            return options.aggregateRows as T[];
          }
          if (sql.includes("from queryable_views")) {
            return [
              {
                ...
                source_tables: options.sourceTables ?? '["stripe_invoices"]'
              }
            ] as T[];
          }
          // freshnessForProviders — the per-provider as_of lookup (lateral over sync_runs).
          if (sql.includes("left join lateral")) {
            return (options.asOf === null ? [] : [{ provider, as_of: options.asOf }]) as T[];
          }
          // connectedSources — the status-gated sources probe.
          if (sql.includes("status in ('connected', 'degraded')")) {
            return (
              options.sourceStatus === null
                ? []
                : [{ id: "src_1", provider, status: options.sourceStatus, last_synced_at: options.asOf }]
            ) as T[];
          }
          return [] as T[];
        },
    ...
    const FRESH_AS_OF = new Date(Date.now() - 60 * 60 * 1000).toISOString(); // 1h ago, within 24h target
    const STALE_AS_OF = "2020-01-01T00:00:00.000Z";
    const dayString = (daysAgo: number) =>
      new Date(Date.now() - daysAgo * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
    const last7DayFilters = [
      { field: "occurred_on", operator: "gte", value: dayString(7) },
      { field: "occurred_on", operator: "lte", value: dayString(0) }
    ];

    it("coalesces an empty revenue window to a confirmed 0 when the source is connected and fresh (the incident case)", async () => {
      const queries: Array<{ sql: string; params?: unknown[] }> = [];
      const handlers = createActionHandlers(
        zeroFakeDb(queries, {
          view: "queryable.vw_revenue_by_source",
          metric: "recognized_revenue",
          aggregateRows: [{ recognized_revenue: null }],
          sourceStatus: "connected",
          asOf: FRESH_AS_OF
        })
      );
      const result = await handlers.run_metric_query?.(
        { metric: "recognized_revenue", filters: last7DayFilters },
        ctx
      );
      expect((result?.data as { rows: unknown }).rows).toEqual([{ recognized_revenue: 0 }]);
      expect((result?.data as Record<string, unknown>)?.noData).toMatchObject({
        reason: "zero_confirmed_fresh"
      });
      expect(result?.caveats).toContain("zero_confirmed_fresh");
      expect(result?.freshness).toMatchObject({ stale: false });
    });

    it("leaves the value null with no marker when the source is stale or disconnected (honesty guard)", async () => {
      // Stale: the provider's last successful sync is far outside the freshness target.
      const staleQueries: Array<{ sql: string; params?: unknown[] }> = [];
      const staleHandlers = createActionHandlers(
        zeroFakeDb(staleQueries, {
          view: "queryable.vw_revenue_by_source",
          metric: "recognized_revenue",
          aggregateRows: [{ recognized_revenue: null }],
          sourceStatus: "connected",
      ...
      const stale = await staleHandlers.run_metric_query?.(
        { metric: "recognized_revenue", filters: last7DayFilters },
        ctx
      );
      expect((stale?.data as { rows: unknown }).rows).toEqual([{ recognized_revenue: null }]);
      expect((stale?.data as Record<string, unknown>)?.noData).toBeUndefined();
      expect(stale?.caveats).not.toContain("zero_confirmed_fresh");

      // Disconnected: freshness reads fine but no connected/degraded source row exists
      // (e.g. the source flipped to error after its last sync). Must NOT become a false $0.
      const disconnectedQueries: Array<{ sql: string; params?: unknown[] }> = [];
      const disconnectedHandlers = createActionHandlers(
        zeroFakeDb(disconnectedQueries, {
          view: "queryable.vw_revenue_by_source",
          metric: "recognized_revenue",
          aggregateRows: [{ recognized_revenue: null }],
          sourceStatus: null,
      ...
      const disconnected = await disconnectedHandlers.run_metric_query?.(
        { metric: "recognized_revenue", filters: last7DayFilters },
        ctx
      );
      expect((disconnected?.data as { rows: unknown }).rows).toEqual([{ recognized_revenue: null }]);
      expect((disconnected?.data as Record<string, unknown>)?.noData).toBeUndefined();
      expect(disconnected?.caveats).not.toContain("zero_confirmed_fresh");
    });

    it("returns a real windowed value untouched with no marker when rows exist", async () => {
      const queries: Array<{ sql: string; params?: unknown[] }> = [];
      const handlers = createActionHandlers(
        zeroFakeDb(queries, {
          view: "queryable.vw_revenue_by_source",
          metric: "recognized_revenue",
          aggregateRows: [{ recognized_revenue: "12000" }],
          sourceStatus: "connected",
          asOf: FRESH_AS_OF
        })
      );
      ...
      expect((result?.data as Record<string, unknown>)?.noData).toBeUndefined();
      expect(result?.caveats).not.toContain("zero_confirmed_fresh");
    });

    it("does not coalesce an empty snapshot-grain metric even when fresh (out of scope)", async () => {
      // stripe_current_paid_subscribers is a point-in-time count: an empty read means the
      // snapshot is absent, NOT that the current count is a confirmed 0.
      const queries: Array<{ sql: string; params?: unknown[] }> = [];
      const handlers = createActionHandlers(
        zeroFakeDb(queries, {
          view: "queryable.vw_stripe_subscription_lifecycle",
          metric: "stripe_current_paid_subscribers",
          aggregateRows: [{ stripe_current_paid_subscribers: null }],
          sourceStatus: "connected",
          asOf: FRESH_AS_OF,
          sourceTables: '["stripe_subscriptions"]'
        })
      );
      const result = await handlers.run_metric_query?.(
        { metric: "stripe_current_paid_subscribers" },
      ...
      expect((result?.data as { rows: unknown }).rows).toEqual([{ stripe_current_paid_subscribers: null }]);
      expect((result?.data as Record<string, unknown>)?.noData).toBeUndefined();
      expect(result?.caveats).not.toContain("zero_confirmed_fresh");
    });
  });
  ...
  // Sessions — the headline GA4 traffic count that already lived in vw_site_traffic (0024
  // added the `sessions` measure + listed it in allowed_measures) but had NO registered
  // metric id reading it, so the desktop Site Analytics hero rendered an em-dash for
  // Sessions. Wired identically to its site-traffic siblings (site_visitors/page_views):
  // additive sum over vw_site_traffic, GA4 traffic authority, ga4_traffic drilldown.
  describe("GA4 sessions metric (vw_site_traffic)", () => {
    it("registers sessions: routes to vw_site_traffic, identity column, additive sum", () => {
      expect(metricView("sessions")).toBe("queryable.vw_site_traffic");
      expect(metricColumn("sessions")).toBe("sessions");
      expect(aggregateExpression("sessions", metricColumn("sessions"))).toBe("sum(sessions)");
      // NON-additive rate metrics session-weight; sessions itself is a plain additive count.
      expect(aggregateExpression("sessions", metricColumn("sessions"))).not.toContain("avg(");
      expect(caveatsForMetric("sessions")).toContain("source_native_attribution_only");
    });

    it("run_metric_query resolves sessions to sum(sessions) over vw_site_traffic", async () => {
      const queries: Array<{ sql: string; params?: unknown[] }> = [];
      const db = ga4TrafficFakeDb(queries);
      const handlers = createActionHandlers(db);

      const result = await handlers.run_metric_query?.(
        { metric: "sessions", view: "queryable.vw_site_traffic" },
        {
          workspaceId: "workspace",
          authority: "tool_agent",
          surface: "api",
      ...
      // Reaching SQL at all proves sessions is in the METRIC_SET gate (FIRST_PHASE_METRICS) —
      // an unregistered metric throws unsupported_metric before any query runs.
      const aggregateQuery = queries.find(
        (entry) => entry.sql.includes("from queryable.vw_site_traffic") && entry.sql.includes("sum(sessions)")
      );
      expect(aggregateQuery?.sql).toBeDefined();
      expect(aggregateQuery?.sql).toContain("sum(sessions) as sessions");
      expect(aggregateQuery?.params).toEqual(["workspace", 500]);
      expect(result?.caveats).toContain("source_native_attribution_only");
    });

    it("rejects a sessions query against a mismatched view before executing SQL", async () => {
      const queries: Array<{ sql: string; params?: unknown[] }> = [];
      const db = ga4TrafficFakeDb(queries);
      const handlers = createActionHandlers(db);

      // vw_posthog_events is a real queryable view but NOT the sessions grain family — the
      // metric→view guard rejects it before any SQL is built.
      await expect(
        handlers.run_metric_query?.(
          { metric: "sessions", view: "queryable.vw_posthog_events" },
          {
            workspaceId: "workspace",
            authority: "tool_agent",
            surface: "api",
      ...
      ).rejects.toThrow("unsupported_view_for_metric:sessions:queryable.vw_posthog_events");
      expect(queries).toHaveLength(0);
    });

    it("breaks sessions down by acquisition channel group", async () => {
      const queries: Array<{ sql: string; params?: unknown[] }> = [];
      const db = ga4TrafficFakeDb(queries);
      const handlers = createActionHandlers(db);
      ...
      expect(breakdownSql).toBeDefined();
      expect(breakdownSql).toContain("sum(sessions) as sessions");
      expect(breakdownSql).toContain("session_default_channel_group as session_default_channel_group");
      expect(breakdownSql).toContain("group by session_default_channel_group");
    });
  });
...
// ─────────────────────────────────────────────────────────────────────────────────────────
describe("metricView routing is explicit and total (no default view)", () => {
  it("routes EVERY exposed metric to a view without throwing", () => {
    // FIRST_PHASE_METRICS is the list list_metrics / the tool schemas hydrate from — every id in it
    // must resolve, and site_conversion_rate must resolve EXPLICITLY (it used to be the function's
    // silent fall-through, so any typo'd metric name was served that view's numbers).
    for (const metric of FIRST_PHASE_METRICS) {
      expect(() => metricView(metric)).not.toThrow();
      expect(metricView(metric)).toMatch(/^queryable\.vw_/);
    }
    expect(metricView("site_conversion_rate")).toBe("queryable.vw_site_conversion_rate");
    expect(metricView("signup_count")).toBe("queryable.vw_posthog_events");
    expect(metricView("posthog_event_count")).toBe("queryable.vw_posthog_events");
    expect(metricView("posthog_page_views")).toBe("queryable.vw_posthog_site");
  });

  it("throws unknown_metric for a name the engine does not expose (no fall-through view)", () => {
    expect(() => metricView("no_such_metric")).toThrow(/unknown_metric:no_such_metric/);
    expect(() => metricView("")).toThrow(/unknown_metric/);
  });

  it("run_metric_query / run_breakdown_query gate on METRIC_SET BEFORE view resolution (unsupported_metric contract)", async () => {
    // A typo'd metric must keep the `unsupported_metric:` contract at the action seam — the
    // `unknown_metric:` throw is reserved for a METRIC_SET member with no routing branch. The db
    // double never gets a query: the gate fires before any SQL.
    const queries: string[] = [];
    const db = {
      async one() {
        return null;
    ...
    const handlers = createActionHandlers(db);
    await expect(handlers.run_metric_query?.({ metric: "no_such_metric" }, ctx)).rejects.toThrow(
      /unsupported_metric:no_such_metric/
    );
    await expect(
      handlers.run_breakdown_query?.({ metric: "no_such_metric", groupBy: ["occurred_on"] }, ctx)
    ).rejects.toThrow(/unsupported_metric:no_such_metric/);
...
describe("Meta Ads management handlers (money-safety + audit + dedup)", () => {
  ...
  function metaWriteTestDb(options: {
    ...
    metaSources?: { id: string }[];
    // The account currency served by resolveMetaAccountCurrency's `select distinct currency from
    // meta_ads_campaigns …` (lowercased ISO, as the warehouse stores it). null/undefined → unknown.
    ...
    return {
      async query<T = Record<string, unknown>>(sql: string, params?: unknown[]): Promise<T[]> {
        ...
        // Source auto-resolve: resolveSoleConnectedMetaSourceId lists connected meta_ads sources.
        if (sql.includes("from sources") && sql.includes("provider = 'meta_ads'")) {
          return (options.metaSources ?? [{ id: "src_meta_sole" }]) as T[];
        }
        // Currency-aware budgets: resolveMetaAccountCurrency reads the synced account currency.
        if (sql.includes("from meta_ads_campaigns")) {
          const currency = options.campaignCurrency;
          return (typeof currency === "string" && currency ? [{ currency }] : []) as T[];
        }
        return [] as T[];
      },
  ...
  describe("⌘L source auto-resolve + currency-aware dollar budgets", () => {
    it("auto-resolves the workspace's SOLE connected Meta source when sourceId is omitted", async () => {
      const audits: AuditRow[] = [];
      const db = metaWriteTestDb({ audits, metaSources: [{ id: "src_meta_sole" }] });
      await withGraph(
        () => jsonResponse({ id: "cmp_auto", status: "PAUSED" }),
        async () => {
          const handlers = createActionHandlers(db);
          const result = await handlers.create_meta_campaign?.(
            { name: "Pizza", objective: "OUTCOME_SALES", clientToken: "tok_auto" },
            operatorContext
          );
          expect(result?.data).toMatchObject({ id: "cmp_auto", status: "PAUSED", deduped: false });
          // The audit is written against the RESOLVED source id (no sourceId was supplied).
          const audit = audits.find((row) => row.action === "create_meta_campaign");
          expect(audit?.source_id).toBe("src_meta_sole");
          expect(audit?.status).toBe("succeeded");
        }
      );
    });

    it("errors HONESTLY (never POSTs) when NO Meta source is connected and sourceId is omitted", async () => {
      const db = metaWriteTestDb({ audits: [], metaSources: [] });
      await withGraph(
        () => jsonResponse({ id: "unused", status: "PAUSED" }),
        async (calls) => {
          const handlers = createActionHandlers(db);
          await expect(
            handlers.create_meta_campaign?.({ name: "Pizza", objective: "OUTCOME_SALES" }, operatorContext)
          ).rejects.toThrow(/meta_ads_not_connected/);
          expect(calls).toHaveLength(0);
        }
      );
    });

    it("REFUSES to guess between MULTIPLE connected Meta sources (typed ambiguity, no POST)", async () => {
      const db = metaWriteTestDb({ audits: [], metaSources: [{ id: "src_a" }, { id: "src_b" }] });
      await withGraph(
        () => jsonResponse({ id: "unused", status: "PAUSED" }),
        async (calls) => {
          const handlers = createActionHandlers(db);
          await expect(
            handlers.create_meta_campaign?.({ name: "Pizza", objective: "OUTCOME_SALES" }, operatorContext)
          ).rejects.toThrow(/meta_ads_source_ambiguous/);
          expect(calls).toHaveLength(0);
        }
      );
    });

    it("honors an EXPLICIT sourceId (back-compat) and does not auto-resolve", async () => {
      const audits: AuditRow[] = [];
      // metaSources: [] → if it tried to auto-resolve it would throw; explicit sourceId must bypass that.
      const db = metaWriteTestDb({ audits, metaSources: [] });
      await withGraph(
        () => jsonResponse({ id: "cmp_explicit", status: "PAUSED" }),
        async () => {
          const handlers = createActionHandlers(db);
          const result = await handlers.create_meta_campaign?.(
            { sourceId: "src_explicit", name: "Pizza", objective: "OUTCOME_SALES", clientToken: "tok_explicit" },
            operatorContext
          );
          expect(result?.data).toMatchObject({ id: "cmp_explicit" });
          const audit = audits.find((row) => row.action === "create_meta_campaign");
          expect(audit?.source_id).toBe("src_explicit");
        }
      );
    });

    it("converts a dollar dailyBudgetMajor to account MINOR units (USD $500 → 50000 cents), stays PAUSED, surfaces currency", async () => {
      const audits: AuditRow[] = [];
      const db = metaWriteTestDb({ audits, metaSources: [{ id: "src_meta_sole" }], campaignCurrency: "usd" });
      await withGraph(
        () => jsonResponse({ id: "cmp_usd", status: "PAUSED" }),
        async (calls) => {
          const handlers = createActionHandlers(db);
          const result = await handlers.create_meta_campaign?.(
            { name: "Pizza", objective: "OUTCOME_SALES", dailyBudgetMajor: 500, clientToken: "tok_usd" },
            operatorContext
          );
          // The connector receives integer CENTS; create is hard-coded PAUSED.
          expect(calls[0].body).toMatchObject({ status: "PAUSED", daily_budget: "50000" });
          // Currency surfaced back so the caller can confirm "$500/day (USD)".
          expect(result?.data).toMatchObject({ budgetCurrency: "USD", status: "PAUSED" });
          // INV-6: the audit still never records the raw amount.
          expect(JSON.stringify(audits)).not.toContain("50000");
        }
      );
    });

    it("uses the per-currency minor-unit EXPONENT — JPY is zero-decimal (¥500 → 500, NOT 50000)", async () => {
      const db = metaWriteTestDb({ audits: [], metaSources: [{ id: "src_meta_sole" }], campaignCurrency: "jpy" });
      await withGraph(
        () => jsonResponse({ id: "cmp_jpy", status: "PAUSED" }),
        async (calls) => {
          const handlers = createActionHandlers(db);
          const result = await handlers.create_meta_campaign?.(
            { name: "Sushi", objective: "OUTCOME_SALES", dailyBudgetMajor: 500, clientToken: "tok_jpy" },
            operatorContext
          );
          expect(calls[0].body).toMatchObject({ daily_budget: "500" });
          expect(result?.data).toMatchObject({ budgetCurrency: "JPY" });
        }
      );
    });

    it("handles THREE-decimal currencies (KWD ×1000: 5 → 5000)", async () => {
      const db = metaWriteTestDb({ audits: [], metaSources: [{ id: "src_meta_sole" }], campaignCurrency: "kwd" });
      await withGraph(
        () => jsonResponse({ id: "cmp_kwd", status: "PAUSED" }),
        async (calls) => {
          const handlers = createActionHandlers(db);
          await handlers.create_meta_campaign?.(
            { name: "Kuwait", objective: "OUTCOME_SALES", dailyBudgetMajor: 5, clientToken: "tok_kwd" },
            operatorContext
          );
          expect(calls[0].body).toMatchObject({ daily_budget: "5000" });
        }
      );
    });

    it("REFUSES a dollar budget when the account currency is UNKNOWN (no guess, no 100× risk, no POST)", async () => {
      const db = metaWriteTestDb({ audits: [], metaSources: [{ id: "src_meta_sole" }], campaignCurrency: null });
      await withGraph(
        () => jsonResponse({ id: "unused", status: "PAUSED" }),
        async (calls) => {
          const handlers = createActionHandlers(db);
          await expect(
            handlers.create_meta_campaign?.(
              { name: "Pizza", objective: "OUTCOME_SALES", dailyBudgetMajor: 500 },
              operatorContext
            )
          ).rejects.toThrow(/meta_account_currency_unknown/);
          expect(calls).toHaveLength(0);
        }
      );
    });

    it("REJECTS passing BOTH cents and dollars for one budget (no silent double-convert)", async () => {
      const db = metaWriteTestDb({ audits: [], metaSources: [{ id: "src_meta_sole" }], campaignCurrency: "usd" });
      await withGraph(
        () => jsonResponse({ id: "unused", status: "PAUSED" }),
        async (calls) => {
          ...
          await expect(
            handlers.create_meta_campaign?.(
              { name: "Pizza", objective: "OUTCOME_SALES", dailyBudget: 50000, dailyBudgetMajor: 500 },
              operatorContext
            )
          ).rejects.toThrow(/budget_units_ambiguous/);
          expect(calls).toHaveLength(0);
        }
      );
    });

    it("leaves an existing MINOR-UNITS caller unchanged (cents pass straight through; NO currency lookup needed)", async () => {
      const audits: AuditRow[] = [];
      // campaignCurrency intentionally UNSET — the cents path must not require a currency.
      const db = metaWriteTestDb({ audits, metaSources: [{ id: "src_meta_sole" }] });
      await withGraph(
        () => jsonResponse({ id: "cmp_cents", status: "PAUSED" }),
        async (calls) => {
          const handlers = createActionHandlers(db);
          await handlers.create_meta_campaign?.(
            { sourceId: "src_meta", name: "Pizza", objective: "OUTCOME_SALES", dailyBudget: 5000, clientToken: "tok_cents" },
            operatorContext
          );
          expect(calls[0].body).toMatchObject({ daily_budget: "5000" });
          // No budgetCurrency surfaced because no dollar amount was converted.
          const audit = audits.find((row) => row.action === "create_meta_campaign");
          expect(audit?.status).toBe("succeeded");
        }
      );
    ...
    it("wires the SAME dollar conversion into create_meta_ad_set (USD $50 → 5000 cents, PAUSED)", async () => {
      const db = metaWriteTestDb({ audits: [], metaSources: [{ id: "src_meta_sole" }], campaignCurrency: "usd" });
      await withGraph(
        () => jsonResponse({ id: "adset_usd", status: "PAUSED" }),
        async (calls) => {
          const handlers = createActionHandlers(db);
  ...
  it("forwards videoUrl through create_meta_creative to the CLI transport", async () => {
    const audits: AuditRow[] = [];
    const dir = mkdtempSync(join(tmpdir(), "analytical-meta-video-cli-"));
    const script = join(dir, "fake-meta-cli.mjs");
    writeFileSync(
      script,
      "console.log(JSON.stringify({ id: 'creative-video-1', status: null }));\n",
      "utf8"
    );
    const db = metaWriteTestDb({
      audits,
  ...
  // R1: every budget update first reads the entity's CURRENT budget type (one node GET). Graph
  // returns budgets as strings and reports an unused budget as "0" or omits it.
  const budgetRead =
    (fields: Record<string, string>, post: () => Response = () => jsonResponse({ success: true })) =>
    (call: GraphCall): Response =>
      call.method === "POST" ? post() : jsonResponse({ id: "120000000000555", ...fields });
  const DAILY_ENTITY = { daily_budget: "4000", lifetime_budget: "0" };
  const LIFETIME_ENTITY = { lifetime_budget: "80000", daily_budget: "0" };

  it("updates a daily budget INLINE (POST /{id} daily_budget only, NO status) and audits budget_present WITHOUT the amount/token", async () => {
    const audits: AuditRow[] = [];
    const db = metaWriteTestDb({ audits });
    await withGraph(
      budgetRead(DAILY_ENTITY),
      async (calls) => {
        const handlers = createActionHandlers(db);
        ...
        // R1 pre-read: ONE node GET for the budget fields only, BEFORE the write.
        expect(calls).toHaveLength(2);
        expect(calls[0].method).toBe("GET");
        const read = new URL(calls[0].url);
        expect(read.pathname).toBe("/v25.0/120000000000555");
        expect(read.searchParams.get("fields")).toBe("id,daily_budget,lifetime_budget");
        // INLINE node POST — daily_budget only, NEVER a status field (money-safety).
        expect(calls[1].url).toBe("https://graph.facebook.com/v25.0/120000000000555");
        expect(calls[1].method).toBe("POST");
        expect(calls[1].body).toEqual({ daily_budget: "5000" });
        expect(calls[1].body).not.toHaveProperty("status");
        expect(calls[1].authorization).toBe("Bearer secret-meta-token");
        expect(result?.data).toMatchObject({ id: "120000000000555", entity: "campaign", updated: true });

        // INVARIANT 6: audit stores presence + ids, NEVER the token or the raw amount.
        const audit = audits.find((row) => row.action === "update_meta_budget");
        expect(audit?.status).toBe("succeeded");
        expect(audit?.actor_type).toBe("operator");
        expect(audit?.details).toMatchObject({
          entity: "campaign",
          entity_id: "120000000000555",
          budget_present: true
        });
        const serialized = JSON.stringify(audits);
        expect(serialized).not.toContain("secret-meta-token");
  ...
  it("updates a LIFETIME budget INLINE (POST /{id} lifetime_budget only) and audits budget_kind WITHOUT the amount", async () => {
    ...
    await withGraph(
      ...
      async (calls) => {
        ...
        expect(calls[1].url).toBe("https://graph.facebook.com/v25.0/120000000000556");
        expect(calls[1].body).toEqual({ lifetime_budget: "91234" });
  ...
  it("R1: refuses a budget-TYPE mismatch in BOTH directions after ONE read — NO POST, audited without amounts", async () => {
    for (const [entityFields, input, current] of [
      // The dangerous direction: a lifetime-sized amount sent as a DAILY budget to a lifetime entity.
      [LIFETIME_ENTITY, { dailyBudget: 300000 }, "lifetime"],
      [DAILY_ENTITY, { lifetimeBudget: 300000 }, "daily"]
    ] as const) {
      const audits: AuditRow[] = [];
      const db = metaWriteTestDb({ audits });
      await withGraph(budgetRead(entityFields), async (calls) => {
        await expect(
          createActionHandlers(db).update_meta_budget?.(
            { sourceId: "src_meta", entityId: "120000000000555", entity: "adset", ...input },
            operatorContext
          )
        ).rejects.toMatchObject({ code: "budget_kind_mismatch", retryable: false });
        ...
        expect(audit?.details).toMatchObject({
          entity: "adset",
          entity_id: "120000000000555",
          budget_kind: "dailyBudget" in input ? "daily" : "lifetime",
          current_budget_kind: current,
          error_code: "budget_kind_mismatch"
        });
        const serialized = JSON.stringify(audits);
  ...
  it("R1: fails CLOSED with budget_kind_unknown when the entity has no budget of its own, or reports both — NO POST", async () => {
    const entities: Array<Record<string, string>> = [
      {}, // e.g. a campaign whose budgets live on its ad sets: Graph omits both fields
      { daily_budget: "0", lifetime_budget: "0" },
      { daily_budget: "4000", lifetime_budget: "80000" }
    ];
    for (const entityFields of entities) {
      const audits: AuditRow[] = [];
      const db = metaWriteTestDb({ audits });
      await withGraph(budgetRead(entityFields), async (calls) => {
        await expect(
        ...
        ).rejects.toMatchObject({ code: "budget_kind_unknown", retryable: false });
        expect(calls.map((call) => call.method)).toEqual(["GET"]);
        expect(audits.find((row) => row.action === "update_meta_budget")?.details).toMatchObject({
          current_budget_kind: "unknown",
          error_code: "budget_kind_unknown"
        });
      });
    }
  });
  ...
  it("R1: a failed budget-type read writes NOTHING and is audited", async () => {
    ...
    const db = metaWriteTestDb({ audits });
    await withGraph(
      (call) => (call.method === "POST" ? jsonResponse({ success: true }) : jsonResponse({ error: { message: "read boom" } }, 500)),
      async (calls) => {
        await expect(
          createActionHandlers(db).update_meta_budget?.(
            { sourceId: "src_meta", entityId: "120000000000555", entity: "campaign", dailyBudget: 5000 },
  ...
  it("R1: the pre-read is COUNTED by the caller's request telemetry (shared per-account request budget)", async () => {
    const db = metaWriteTestDb({ audits: [] });
    await withGraph(budgetRead(DAILY_ENTITY), async () => {
      const telemetry = new MetaAdsRequestTelemetry(5);
      await createActionHandlers(db, { metaAdsRequestTelemetry: telemetry }).update_meta_budget?.(
        { sourceId: "src_meta", entityId: "120000000000555", entity: "campaign", dailyBudget: 5000 },
        operatorContext
      );
      // Exactly the one GET is a metered read; the write POST is not a read-budget request.
      expect(telemetry.snapshot()).toMatchObject({ requestCount: 1 });
    });
  });
  ...
  it("BUDGET GUARD: entity=ad is refused (Meta has no ad-level budget) — NO POST, no spend", async () => {
    ...
    await withGraph(
      ...
      async (calls) => {
        ...
        await expect(
          handlers.update_meta_budget?.(
            { sourceId: "src_meta", entityId: "120000000000555", entity: "ad", dailyBudget: 5000 },
            operatorContext
          )
        ).rejects.toThrow(/unsupported_meta_budget_entity/);
        // The guard fires BEFORE any Graph POST.
        expect(calls).toHaveLength(0);
      }
    );
  });

  it("BUDGET GUARD: a 0 / negative / non-integer budget is refused — NO POST", async () => {
    const audits: AuditRow[] = [];
    const db = metaWriteTestDb({ audits });
    await withGraph(
      () => jsonResponse({ success: true }),
      async (calls) => {
        ...
        for (const bad of [0, -100, 12.5]) {
          await expect(
            handlers.update_meta_budget?.(
              { sourceId: "src_meta", entityId: "120000000000555", entity: "campaign", dailyBudget: bad },
              operatorContext
            )
          ).rejects.toThrow(/invalid_daily_budget/);
        }
        expect(calls).toHaveLength(0);
      }
    );
  ...
  it("audits a budget-update failure (non-retryable) and re-throws, recording budget_present but not the amount", async () => {
    const audits: AuditRow[] = [];
    const db = metaWriteTestDb({ audits });
    await withGraph(
      budgetRead(DAILY_ENTITY, () => jsonResponse({ error: { message: "boom" } }, 500)),
      async () => {
        const handlers = createActionHandlers(db);
        await expect(
          handlers.update_meta_budget?.(
            { sourceId: "src_meta", entityId: "120000000000555", entity: "adset", dailyBudget: 7500 },
            operatorContext
          )
        ).rejects.toMatchObject({ retryable: false });
        const audit = audits.find((row) => row.action === "update_meta_budget");
        expect(audit?.status).toBe("failed");
        expect(audit?.details).toMatchObject({ entity: "adset", budget_present: true });
        const serialized = JSON.stringify(audits);
  ...
  // ── run_meta_live_insights — the live ⌘L Meta answer path ────────────────────────────────
  // A whole-window AGGREGATE Graph read (never the local queryable store, never the CLI).

  /** One ad-grain insights row as Graph returns it (purchase objective, canonical pixel event). */
  function liveAdRow(overrides: Record<string, unknown> = {}): Record<string, unknown> {
    return {
      ad_id: "ad_1",
      ad_name: "Blue Hero",
      adset_id: "as_1",
  ...
  it("run_meta_live_insights is a tool_agent READ: aggregate ad-grain GET, no audit row, no CLI", async () => {
    const audits: AuditRow[] = [];
    const db = metaWriteTestDb({ audits });
    await withGraph(
      () => jsonResponse({ data: [liveAdRow()] }),
      async (calls) => {
        const handlers = createActionHandlers(db);
        ...
        expect(calls[0].method).toBe("GET");
        const url = new URL(calls[0].url);
        expect(url.pathname).toContain("/act_999/insights");
        expect(url.searchParams.get("level")).toBe("ad");
        expect(url.searchParams.get("date_preset")).toBe("last_30d");
        // Ad-grain identity fields ride the request; the window is an AGGREGATE (no per-day rows).
        expect(url.searchParams.get("fields")).toContain("ad_id,ad_name,adset_id");
        expect(url.searchParams.get("time_increment")).toBeNull();
        expect(url.searchParams.get("action_attribution_windows")).toBe(
          JSON.stringify(["1d_click", "7d_click", "1d_view"])
        );
        expect(result?.ok).toBe(true);
        expect(result?.data).toMatchObject({ level: "ad", count: 1, totalRows: 1 });
        const row = (result?.data as { rows: Array<Record<string, unknown>> }).rows[0];
        // Canonical conversion semantics (headline = 7d_click + 1d_view, value purchase-only),
        // ROAS = conversionValue / spend — the warehouse definition, not raw purchase_roas.
        expect(row).toMatchObject({
          adName: "Blue Hero",
          spend: 120.5,
          resultType: "purchase",
          results: 5,
          conversionValue: 482.5
        });
        expect(row.roas).toBeCloseTo(482.5 / 120.5, 6);
        // Reads never write an audit row, and the token never leaks into the envelope.
        expect(audits).toHaveLength(0);
  ...
  it("run_meta_live_insights sorts by spend desc and truncates to limit with an honest caveat", async () => {
    const db = metaWriteTestDb({ audits: [] });
    await withGraph(
      () =>
        jsonResponse({
          data: [
            liveAdRow({ ad_id: "ad_small", ad_name: "Small", spend: "10" }),
            liveAdRow({ ad_id: "ad_big", ad_name: "Big", spend: "900" }),
            liveAdRow({ ad_id: "ad_mid", ad_name: "Mid", spend: "50" })
          ]
        }),
      async () => {
        const handlers = createActionHandlers(db);
        const result = await handlers.run_meta_live_insights?.(
          { sourceId: "src_meta", limit: 2 },
          { ...operatorContext, authority: "tool_agent" }
        );
        const data = result?.data as {
          rows: Array<{ adName: string }>;
          count: number;
          totalRows: number;
        };
        expect(data.rows.map((r) => r.adName)).toEqual(["Big", "Mid"]);
        expect(data).toMatchObject({ count: 2, totalRows: 3 });
        expect(result?.caveats?.join(" ")).toContain("Truncated to the top 2 of 3");
      }
    );
  });

  it("run_meta_live_insights auto-resolves the sole connected meta source when sourceId is omitted", async () => {
    const base = metaWriteTestDb({ audits: [] });
    const db: InfiniteOsDb = {
      ...base,
      async query<T = Record<string, unknown>>(sql: string, params?: unknown[]): Promise<T[]> {
        if (sql.includes("from sources") && sql.includes("provider = 'meta_ads'")) {
          return [{ id: "src_meta" } as T];
        }
  ...
  it("run_meta_live_insights fails typed when no (or many) meta sources exist and sourceId is omitted", async () => {
    const base = metaWriteTestDb({ audits: [] });
    ...
    await withGraph(
      () => jsonResponse({ data: [] }),
      async (calls) => {
        const handlers = createActionHandlers(withSources([]));
        await expect(
          handlers.run_meta_live_insights?.({}, { ...operatorContext, authority: "tool_agent" })
        ).rejects.toThrow("meta_ads_not_connected");
        const ambiguous = createActionHandlers(withSources([{ id: "src_a" }, { id: "src_b" }]));
        await expect(
          ambiguous.run_meta_live_insights?.({}, { ...operatorContext, authority: "tool_agent" })
        ).rejects.toThrow("meta_ads_source_ambiguous");
        // Neither failure ever reached the Graph API.
        expect(calls).toHaveLength(0);
      }
    );
  });

  it("run_meta_live_insights validates level, preset, and the since/until window before any fetch", async () => {
    const db = metaWriteTestDb({ audits: [] });
    await withGraph(
      () => jsonResponse({ data: [] }),
      async (calls) => {
        ...
        await expect(
          handlers.run_meta_live_insights?.({ sourceId: "src_meta", level: "creative" }, ctx)
        ).rejects.toThrow("unsupported_meta_level");
        await expect(
          handlers.run_meta_live_insights?.({ sourceId: "src_meta", datePreset: "last_90d" }, ctx)
        ).rejects.toThrow("unsupported_meta_date_preset");
        await expect(
          handlers.run_meta_live_insights?.({ sourceId: "src_meta", since: "2026-07-01" }, ctx)
        ).rejects.toThrow("invalid_date_range");
        await expect(
          handlers.run_meta_live_insights?.(
            { sourceId: "src_meta", since: "2026-07-09", until: "2026-07-01" },
            ctx
          )
        ).rejects.toThrow("invalid_date_range");
        expect(calls).toHaveLength(0);
        // A valid explicit window rides time_range (and drops date_preset).
        await handlers.run_meta_live_insights?.(
          { sourceId: "src_meta", since: "2026-07-01", until: "2026-07-09", level: "campaign" },
          ctx
        );
        const url = new URL(calls[0].url);
        expect(url.searchParams.get("time_range")).toBe(
          JSON.stringify({ since: "2026-07-01", until: "2026-07-09" })
        );
