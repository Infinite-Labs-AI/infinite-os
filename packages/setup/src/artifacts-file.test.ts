...
describe("writeSetupArtifactsFile", () => {
  it("preserves valid non-owned providers while replacing setup-owned provider keys", () => {
    const dir = makeTempDir();
    const filePath = join(dir, "ws_1.json");
    writeFileSync(
      filePath,
      JSON.stringify({
        workspaceId: "ws_1",
        infinite: {
          siteSourceKey: "site_public_desktop",
          collectPath: "/infinite/events/collect",
          productionHosts: ["acme.test"],
          staticProxy: "vercel"
        },
        meta: { pixelId: "1234567890" },
        futureProvider: { publicId: "future_public_1" },
        ga4: { measurementId: "G-OLD123" },
        posthog: { projectKey: "phc_stale" }
      })
    );
    ...
    writeSetupArtifactsFile({
      ...
      artifacts: resolvedArtifacts({
        ga4: { measurementId: "G-NEW123", propertyId: null }
      }),
      dir
    });
    ...
    expect(JSON.parse(readFileSync(filePath, "utf8"))).toEqual({
      ...
      meta: { pixelId: "1234567890" },
      futureProvider: { publicId: "future_public_1" },
      ga4: { measurementId: "G-NEW123" }
    });
    expect(readdirSync(dir)).toEqual(["ws_1.json"]);
  });

  it("refuses to overwrite malformed or cross-workspace artifact files", () => {
    const dir = makeTempDir();
    const filePath = join(dir, "ws_1.json");
    const artifacts = resolvedArtifacts({ ga4: { measurementId: "G-NEW123", propertyId: null } });

    for (const existing of ["not-json", JSON.stringify({ workspaceId: "ws_other", infinite: {} })]) {
      writeFileSync(filePath, existing);
      expect(() => writeSetupArtifactsFile({ workspaceId: "ws_1", artifacts, dir })).toThrow(
        /existing artifact file/i
      );
  ...
  it("writes the whitelisted public payload for every captured provider with 0600 perms", () => {
    ...
    expect(buildWorkspaceArtifactsFromResolved("ws_1", JSON.parse(readFileSync(filePath!, "utf8")))).toEqual({
      infinite: {
        siteSourceKey: "site_public_acme",
        collectPath: "/infinite/events/collect",
        productionHosts: ["acme.test", "www.acme.test"],
  ...
  it("persists an explicit mirror-only production host allowlist", () => {
    const dir = makeTempDir();
    const artifacts = {
      ...resolvedArtifacts({ ga4: { measurementId: "G-ACME123", propertyId: null } }),
      productionHosts: ["acme.test"]
    };
    const filePath = writeSetupArtifactsFile({ workspaceId: "ws_1", artifacts, dir });
    ...
    expect(buildWorkspaceArtifactsFromResolved("ws_1", artifacts)).toEqual({
      productionHosts: ["acme.test"],
      ga4: { measurementId: "G-ACME123" }
    });
  });
