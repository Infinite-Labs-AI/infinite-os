...
import {
  buildProviderConfirmations,
  formatInfiniteCollectionStatus,
...
describe("formatProviderConfirmation", () => {
  ...
  it("does not advertise an artifact-less install when GA4 connected without a Measurement ID", () => {
    ...
    expect(line).toBe(
      "✓ GA4 connected. Next: re-run `infinite local setup` to capture its Measurement ID — nothing to install yet."
    );
    expect(line).not.toContain("npx infinite-tag install");
  });
  ...
  it("surfaces the resume command for a paused provider when the run id is known", () => {
    ...
    expect(line).toBe(
      "→ GA4 not connected yet — setup paused for a step in your browser. " +
        "Next: finish that step, then run `infinite local setup resume run_ga4`."
    );
  });
  ...
  it("points a paused provider at setup status when no run id is known", () => {
    ...
    expect(line).toBe(
      "→ PostHog not connected yet — setup paused for a step in your browser. " +
        "Next: finish that step, then run `infinite local setup status` to get the resume command."
    );
  });
  ...
  it("reports a failed provider with the failing phase detail and a retry next step", () => {
    ...
    expect(line).toBe(
      "✗ X failed — X API rejected the bearer token. Next: fix the issue above, then run `infinite local setup` again."
    );
  });
  ...
  it("reports a failed provider without a detail when no phase carries one", () => {
    ...
    expect(line).toBe("✗ GA4 failed. Next: fix the issue above, then run `infinite local setup` again.");
  });
  ...
  it("marks a provider without a run as skipped", () => {
    ...
    expect(line).toBe(
      "– PostHog skipped — nothing was set up or captured for it in this run. Next: run `infinite local setup` again to include it."
    );
  });
});

describe("formatInfiniteCollectionStatus", () => {
  it("states the verification and proxy gate when no source was provisioned", () => {
    expect(formatInfiniteCollectionStatus()).toBe(
      "Infinite website collection and shared GA4/PostHog browser events remain disabled. Verify the website domain and supply at least one explicit production host; Infinite also requires a supported same-origin proxy."
    );
  });

  it("confirms only the public source and host handoff", () => {
    expect(
      formatInfiniteCollectionStatus({
    ...
    ).toBe("Infinite website collection is ready for acme.test through /infinite/events/collect.");
  });
});
...
describe("buildProviderConfirmations", () => {
  ...
  it("emits one confirmation per selected provider, resolving paused resume ids from active runs", () => {
    ...
    expect(lines[1]).toContain("infinite local setup resume run_posthog");
  });
  ...
  it("marks selected providers that never ran as skipped and ignores unselected ones", () => {
    ...
    expect(lines).toEqual([
      "– GA4 skipped — nothing was set up or captured for it in this run. Next: run `infinite local setup` again to include it.",
      "– PostHog skipped — nothing was set up or captured for it in this run. Next: run `infinite local setup` again to include it."
    ]);
  });
});
