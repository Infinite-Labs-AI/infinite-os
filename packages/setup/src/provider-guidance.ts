...
function posthogApiKeyBody(): GuidanceBody {
  return {
    ...
    confirm: "paste it here when prompted, or run `infinite local setup resume <run_id>` and paste/import it.",
...
function posthogSignupBody(): GuidanceBody {
  return {
    ...
    confirm: "run `infinite local setup resume <run_id>` once you reach a PostHog project home page.",
...
function xBillingBody(): GuidanceBody {
  return {
    ...
    confirm: "run `infinite local setup resume <run_id>` after you have the token.",
...
export function providerGuidance(
  ...
  if (runId) {
    lines.push(`Resume any time with: infinite local setup resume ${runId}`);
  }
