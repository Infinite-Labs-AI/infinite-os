...
export function formatInfiniteCollectionStatus(artifact?: InfinitePublicArtifact | null): string {
  if (!artifact) {
    return "Infinite website collection and shared GA4/PostHog browser events remain disabled. Verify the website domain and supply at least one explicit production host; Infinite also requires a supported same-origin proxy.";
  }
  return `Infinite website collection is ready for ${artifact.productionHosts.join(", ")} through ${artifact.collectPath}.`;
}
...
export function formatProviderConfirmation(input: ProviderConfirmationInput): string {
  ...
  if (!input.run) {
    return `– ${label} skipped — nothing was set up or captured for it in this run. Next: run \`infinite local setup\` again to include it.`;
  }
...
function formatSucceeded(
  ...
  if (!installCommand) {
    return `${status} Next: re-run \`infinite local setup\` to capture its ${PROVIDER_INSTALL_ID_NOUNS[provider]} — nothing to install yet.`;
  }
...
function formatPaused(label: string, runId: string | undefined): string {
  const resume = runId
    ? `run \`infinite local setup resume ${runId}\``
    : "run `infinite local setup status` to get the resume command";
...
function formatFailed(label: string, phases: Partial<Record<string, PhaseResult>>): string {
  ...
  return `${status} Next: fix the issue above, then run \`infinite local setup\` again.`;
}
