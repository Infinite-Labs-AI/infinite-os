import type { InfinitePublicArtifact } from "infinite-tag";
 ...
 * (site source key / measurement id / project key / pixel id) — secrets never enter this string.
 */
...
export interface InstallCommandArtifacts {
  productionHosts?: string[] | null;
  infinite?: InfinitePublicArtifact | null;
...
export function buildInstrumentInstallCommand(input: {
  ...
  const siteSourceKey = cleanValue(input.artifacts?.infinite?.siteSourceKey);
  const collectPath = cleanValue(input.artifacts?.infinite?.collectPath);
  const productionHosts = (
    input.artifacts?.productionHosts ?? input.artifacts?.infinite?.productionHosts ?? []
  )
    .map((host) => cleanValue(host))
    .filter((host): host is string => host !== null);
  const hasInfiniteArtifact = Boolean(siteSourceKey && collectPath && productionHosts.length > 0);
  const hasMirrorArtifact = Boolean(
    cleanValue(input.artifacts?.ga4?.measurementId) ||
      cleanValue(input.artifacts?.posthog?.projectKey)
  );
  if (siteSourceKey && collectPath && productionHosts.length > 0) {
    flags.push(`--infinite-site-source-key ${quoteForShell(siteSourceKey)}`);
    flags.push(`--infinite-collect-path ${quoteForShell(collectPath)}`);
  }
  if (hasInfiniteArtifact || hasMirrorArtifact) {
    for (const host of [...new Set(productionHosts)]) {
      flags.push(`--infinite-production-host ${quoteForShell(host)}`);
    }
  }
  if (siteSourceKey && collectPath && productionHosts.length > 0) {
    if (input.artifacts?.infinite?.staticProxy === "vercel") {
      flags.push("--infinite-static-proxy vercel");
    }
    if (input.artifacts?.infinite?.consentMode) {
      flags.push(
        `--infinite-consent-mode ${input.artifacts.infinite.consentMode === "not_required" ? "not-required" : "required"}`
      );
    }
  }
