...
function buildGa4DetectionOutcome(
  ...
  if (selection.stream?.measurementId) {
    return {
      ...
      syncDefaults: { mode: "incremental", refreshWindowDays: 14 }
    };
  }
  ...
  input.note?.(
    stream.measurementId
      ...
      : "GA4: web stream ready — the Measurement ID is still pending from Google. " +
          "Next: re-run `infinite local setup` to capture it once it appears."
  );
...
export async function syncGa4Contract(
  ...
  const syncDefaults = detected.syncDefaults ?? { mode: "incremental" as const, refreshWindowDays: 14 };
  const envelope = await ctx.actions.execute(
    "start_source_sync",
    {
      sourceId: state.sourceId,
