...
import { buildProviderConfirmations, formatInfiniteCollectionStatus } from "./confirmations.js";
import { writeSetupArtifactsFile } from "./artifacts-file.js";
import { buildInstrumentInstallCommand, type InstallCommandArtifacts } from "./install-command.js";
import {
...
export interface LiveSetupExecutionOptions {
  ...
  prompt: Prompter;
  /** Browser-safe source handoff supplied by the desktop control plane; setup never provisions it. */
  infiniteArtifact?: InfinitePublicArtifact;
  repoRoot?: string;
...
async function executeLiveSetupOnboarding(
  ...
  const installArtifacts: InstallCommandArtifacts = {
    ...resolvedPublicArtifacts,
    ...(options.infiniteArtifact ? { infinite: options.infiniteArtifact } : {})
  };
  const installCommand = buildInstrumentInstallCommand({
    workspaceId: options.workspaceId,
    artifacts: installArtifacts
  });
  options.prompt.note(formatInfiniteCollectionStatus(options.infiniteArtifact));
