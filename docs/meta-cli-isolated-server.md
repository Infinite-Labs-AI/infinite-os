...
connected source's `account_external_id` matches its `adAccountId`, requires a CLI
transport for mutations, then binds the
...
stdout at 128 KiB and stderr at 8 KiB, hard-kills the child process group on a bound
failure, waits for child `close` with a bounded fallback, and removes the directory
on success and failure. Provider stderr is not returned to callers because it can
echo a token. Successful stdout containing the active token or EAA-shaped token
material is rejected before parsing. Existing local execution remains the default when the option is
omitted.
...
and provide the workspace encryption key. That per-request key is passed through
the Meta read/write credential resolver; no tenant key is set in `process.env`.
Stored-token Meta reads retain the
