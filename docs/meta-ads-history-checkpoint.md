...
- URL-only image, thumbnail and carousel creative slots are retained after signed-URL removal.
  ...
  `{slotKey, kind, providerAssetId, slotFingerprint, sourceUrl, sourceLocator}`. `sourceUrl` remains
  `null`; `sourceLocator` stores only `{host, path}` and `slotFingerprint` is non-reversible.
...
2. H5 must consume `slotFingerprint` + `sourceLocator` as advisory slot identity only, then resolve a
   fresh provider download URL under the workspace credential.
