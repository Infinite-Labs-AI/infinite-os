...
#   4. Runs `infinite local setup`. The first run self-installs workspace deps
#      and builds the CLI (you'll see the build progress bar), then configures
#      the local Infinite engine — so there is no separate build step here.
#
...
#   --skip-setup    Don't run `infinite local setup` at the end
  ...
  case "$1" in
    ...
    -h|--help)
      ...
      if [ -f "$0" ]; then
        ...
        printf 'Installs Infinite to ~/.infinite/app, puts `infinite` on your PATH, then runs `infinite local setup`.\n'
      fi
      exit 0 ;;
...
if [ "$RUN_SETUP" != true ]; then
  ...
  log_info "Next:  infinite local setup    (the first run builds the CLI, then configures the local engine)"
  exit 0
fi

if [ "$HAS_TTY" = true ]; then
  log_info "Running 'infinite local setup' (first run builds the CLI — this can take ~20-30s)…"
  printf "\n"
  if [ -t 0 ]; then
    "$INSTALL_DIR/infinite" local setup
  else
    "$INSTALL_DIR/infinite" local setup < /dev/tty
  fi
else
  log_success "Install complete."
  log_info "No terminal detected, so setup was skipped."
  log_info "Open a new terminal and run:  infinite local setup"
fi
