...
#    local home paths, live workspace ids (real ids are exactly 16 hex
#    chars, so fixtures like ws_1 / ws_test / ws_REDACTED won't match), or
#    real Meta ad-account ids (real act_ ids are 12+ digits; synthetic test
#    ids in this repo are 10 digits, so act_1234567890 won't match — a live
#    Graph API capture pasted into a fixture WILL).
#    This script is excluded because it must name the patterns it hunts.
# ---------------------------------------------------------------------------
personal_hits="$(git grep -nE 'alchemistchaos@protonmail|/Users/chaosalchemist|ws_[0-9a-f]{16}|act_[0-9]{12,}' -- ':(exclude)scripts/ci/repo-tripwire.sh' || true)"
if [[ -n "$personal_hits" ]]; then
  echo "$personal_hits" >&2
  fail "personal data (owner email / home path / live workspace id / real ad-account id) found in tracked files (see list above)"
fi
...
# 6. PUBLIC-MODE: commit author/committer emails must be on the public allowlist.
