...
- **We publish an npm package: [`infinite-tag`](https://www.npmjs.com/package/infinite-tag) (= `packages/instrument`).** It's the founder-run installer that adds the GA4 / PostHog / X tracking tags into the *user's own website repo* (`npx infinite-tag install`, public keys only). `infinite local setup` surfaces a pre-filled `npx infinite-tag install …` after analytics connect. When anything touches "install the tag on the user's site," remember this package exists — don't reinvent it. (See AGENTS.md → repo layout.)
