...
**Infinite OS** is the open-core local engine bundled into the macOS-only Infinite Desktop app. It
connects a user's data sources (GA4, PostHog, Stripe, Meta, Shopify, X) to the app's embedded local
database and exposes governed tools. The supported customer distribution is the signed Desktop app;
the source/docker topology in this repository is for maintainers and contributors.
...
> **`packages/instrument` is the published npm package [`infinite-tag`](https://www.npmjs.com/package/infinite-tag)** (NOT private). It's the founder-run installer that adds the GA4 / PostHog / X tracking tags into the *user's own website repo* (via `npx infinite-tag install` — public keys only, idempotent, reversible). `infinite local setup` prints a pre-filled `npx infinite-tag install …` after analytics connect; GA4 also auto-installs it in-process (`packages/setup/src/provisioner.ts` → `import("infinite-tag")`). Don't reinvent tag-install logic — it lives here and is published via `.github/workflows/publish.yml`.
...
- **Desktop owns customer updates.** The signed app reads the public Desktop release feed and brings
  its frozen engine and CLI with it. Source-checkout SHA update behavior exists for maintainers only;
  never describe it as the customer distribution contract.
...
> [!WARNING]
> **`infinite-os` ≠ `infinite-tag` — do not confuse them.** The **`infinite-os` npm package**
> (`packages/desktop-installer`) installs the signed Desktop app. A **git tag** (`vX.Y.Z`) is a
> label on a commit in *this* repo that marks a source release. **`infinite-tag`** is our
...
- **Activation is separate and gated.** Going live is a distinct
  `infinite local meta <obj> activate <id>` → `set_meta_entity_status status=ACTIVE`, behind a
