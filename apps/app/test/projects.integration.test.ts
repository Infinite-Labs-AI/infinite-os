...
describe.skipIf(!IT_URL)("projects e2e (live postgres)", () => {
  ...
  it("deleteProject removes the workspace + ALL child rows and leaves others untouched", async () => {
    ...
    // The PostHog retention footprint (0063 rollups + the 0064 prune watermark). The inventory below
    // is discovered from the catalog, but it can only catch a table the delete list misses if the
    // doomed workspace actually HAS a row there — so seed one. posthog_prune_watermarks matters most:
    // its FK to sources has no ON DELETE, so omitting it doesn't leave stray rows, it aborts the
    // entire deleteProject transaction and makes every previously-pruned workspace undeletable.
    await db.query(
