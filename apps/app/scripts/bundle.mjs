...
import { createBundleProvenance } from "./bundle-provenance.mjs";

const here = dirname(fileURLToPath(import.meta.url));
const appRoot = join(here, ".."); // apps/app
const outDir = join(appRoot, "dist", "bundle");
const daemonPath = join(outDir, "daemon.mjs");
...
// Bind the provenance claim to the exact daemon bytes. The stamp is a required build artifact:
// a release must fail before packaging if it cannot identify or hash the engine it would ship.
const provenance = createBundleProvenance({ daemonPath, appRoot });
writeFileSync(
