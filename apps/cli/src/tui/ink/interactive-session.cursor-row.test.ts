...
import { composerCursorLayout, composerNativeCursorPosition, createActivityAwareCursorBlink, renderInkInteractiveSessionToString, wouldTriggerInkFullscreen } from "./interactive-session.js";
...
describe("activity-aware native cursor blinking", () => {
  afterEach(() => {
    vi.useRealTimers();
  });

  it("stays steady while typing and resumes blinking only after the idle delay", () => {
    vi.useFakeTimers();
    const writes: string[] = [];
    const controller = createActivityAwareCursorBlink((sequence) => writes.push(sequence), 500);

    controller.activity();
    expect(writes).toEqual(["\x1b[2 q"]); // steady block during input

    vi.advanceTimersByTime(400);
    controller.activity();
    vi.advanceTimersByTime(499);
    expect(writes).toEqual(["\x1b[2 q"]);

    vi.advanceTimersByTime(1);
    expect(writes).toEqual(["\x1b[2 q", "\x1b[1 q"]); // blinking block when idle

    controller.dispose();
    expect(writes.at(-1)).toBe("\x1b[0 q"); // restore the terminal's configured default
  });
});
