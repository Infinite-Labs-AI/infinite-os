...
describe("in-chat /connect wizard (#20) — live PTY flow (skipped on CI)", () => {
  ...
  it.skipIf(process.env.CI === "true")(
    "/connect ga4 shows the `infinite local setup` note (no field loop, no LLM dispatch)",
    { timeout: 30_000 },
    async () => {
      const input = ttyInput();
      const output = ttyOutput();
      ...
      const session = runInkInteractiveSession({
        ...
        connectWizard: (line) =>
          line.includes("connect ga4")
            ? { kind: "note", text: "GA4 quick-connect opens a browser — run `infinite local setup` in your terminal to connect GA4." }
            : { kind: "none" },
        async onSubmitLine(line) {
          dispatched.push(line);
        ...
        const session = runInkInteractiveSession({
          ...
          connectWizard: (line) =>
            line.includes(`connect ${provider}`)
              ? { kind: "note", text: `Run \`infinite local connect ${provider}\` in your terminal for now.` }
              : { kind: "none" },
          async onSubmitLine(line) {
            dispatched.push(line);
            return { exit: true, messages: [{ role: "assistant", text: "x" }] };
