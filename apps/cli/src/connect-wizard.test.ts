...
describe("decideConnectWizard — scope + routing", () => {
  ...
  it("DEFERS GA4: /connect ga4 and google_analytics_4 show the `infinite local setup` note (no wizard)", () => {
    for (const token of ["ga4", "google_analytics_4", "google"]) {
      ...
      expect(decision.text).toContain("infinite local setup");
  ...
  it("GUARDS shopify: /connect shopify shows the terminal fallback note (no wizard)", () => {
    for (const provider of ["shopify"]) {
      ...
      expect(decision.text).toContain("infinite local connect");
