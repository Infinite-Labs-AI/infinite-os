...
export interface CliEnv {
  ...
  /** Explicit engine opt-in/out for bare/product invocations: "local" | "cloud". */
  GROWTH_OS_DEFAULT_TARGET?: string;
...
const HOME_INVENTORY_LABEL_MAX_CHARS = 80;

export function homeInventoryProviderLabel(provider: string): string {
  const safeProvider = boundedTerminalText(
    provider,
    HOME_INVENTORY_LABEL_MAX_CHARS,
    "Unknown"
  );
  return (
    HOME_INVENTORY_PROVIDER_LABELS[safeProvider] ??
    safeProvider
      .split(/[_\s]+/)
      .filter(Boolean)
...
// Help text lives in ./help-text.ts, SPLIT into product vs local surfaces
// (design §6.6). `helpText()` stays exported for existing callers and today
// still returns the full engine help; the Phase-3 entry rewrite points the
// product surface at `productHelpText()`.
export { localHelpText, productHelpText, productUpdateText } from "./help-text.js";
...
// ── Interactive entry routing (`infinite` with no command) ───────────────────
...
async function runInteractiveEntry(env: CliEnv): Promise<void> {
  ...
  if (mode === "onboarding") {
    ...
    // so scripts/wrappers observe the failure. Never a silent local turn.
    process.exitCode = 1;
    return;
  }
...
// LIVE bridge probe (spec §6.4): a readable descriptor AND `/v1/status`
// reachable + `ready`. Any failure — no descriptor, unsafe file, dead port,
// not-ready — means "no live bridge" for routing purposes; the precise
// diagnosis surfaces later (onboarding guidance or `infinite app status`).
async function desktopLiveBridgeReady(env: CliEnv): Promise<boolean> {
  try {
    const resolved = resolveLiveBridge(env);
    if (!resolved) {
      return false;
    }
    return (await resolved.client.status()).ready;
  } catch {
    return false;
  }
}
...
function readDesktopOnboardingState(env: CliEnv): OnboardingState | null {
  try {
    const raw = readFileSync(
      join(infiniteOsHome(env as NodeJS.ProcessEnv), "desktop-cmdl", "state.json"),
      "utf8"
    );
...
async function runDesktopInteractiveEntry(env: CliEnv): Promise<void> {
  ...
  // re-checks `/v1/status` through the session runner below (spec §6.8).
  const resolved = resolveLiveBridge(env);
  if (!resolved) {
  ...
  // Per-turn preflight + session owner: re-reads `bridge.json` (fresh client on
  ...
  if (shouldUseInkInteractiveSession(input, output, env)) {
    ...
    await runInkInteractiveSession({
      ...
      async onSubmitLine(line, onProgress) {
        ...
        // Surface any redacted write confirmations to the Ink component's
        // in-session y/N gate (rendered AFTER this turn's answer). None → [].
        return outcome.pendingConfirmations?.length
          ? { messages: [], pendingConfirmations: outcome.pendingConfirmations }
          : { messages: [] };
      }
    });
    return;
  ...
  // Plain (non-Ink) terminal: the headless loop, rendering streamed events
  // through a progress reporter.
  const reporter = createCliProgressReporter(output, env, {
    promptPlaceholder: "Type a message, /help, or /exit."
  });
  const controller: DesktopTurnControllerLike = {
    recordProgressEvent: (event) => reporter.progress(event)
  };
  const interactiveIo: DesktopInteractiveIo = {
    writeOut: (text) => {
      output.write(text);
    },
    writeErr: (text) => {
  ...
  try {
    await runDesktopInteractive(env as NodeJS.ProcessEnv, interactiveIo, {
      // Adapt the session runner onto the loop's turn-source seam. The runner
      // OWNS the session (per-turn preflight + reset-on-scope-change), so the
      // loop's threaded sessionId is ignored. The serial readline loop cannot
      ...
      // Wire the in-session write-confirmation seam. When a turn surfaces
      // `requires_confirmation` actions, each is rendered + prompted on this
      // same readline instance and — only on an explicit `y`/`yes` — confirmed
      // via the runner (which targets the client that ran the originating
      // turn). The single shared `rl` is safe: the handler is awaited between
      // line reads, never concurrently. Non-TTY declines.
      confirmation: {
        client: { confirm: (confirmInput) => runner.confirm(confirmInput) },
        io: {
      ...
      readLines: () => rl[Symbol.asyncIterator](),
      signal: turnAbort.signal
    });
  } finally {
    rl.close();
...
function desktopHelpMessage(): Msg {
  return {
    kind: "slash",
    role: "system",
    text: "Type a message to run a turn. /exit to leave."
  } as Msg;
}

function desktopBusyMessage(): Msg {
  return {
    kind: "slash",
    role: "system",
    text: "A turn is already running. Wait for it to finish, then try again."
  } as Msg;
}

// The §6.6 CANONICAL entry routing order, implemented in sequence:
//   (1) normalize aliases + detect the explicit `local` namespace;
//   (2) product `help`/`version` + Desktop-oriented `update`;
...
//   (6) interactive mac with no bridge → onboarding (launch + poll Desktop);
//   (7) non-TTY / non-mac with no bridge → exit non-zero with guidance;
//   (8) NEVER fall through to local unless `local` was explicit.
export async function runCli(
  args = process.argv.slice(2),
  env: CliEnv = process.env
): Promise<void> {
  // (1) Normalize + detect explicit `local`.
  ...
  if (normalizedArgs.length === 0) {
    ...
    await runInteractiveEntry(env);
    return;
  }

  // `infinite local [...]` — the ONLY road to the local engine (§6.6 step 8).
  // It bypasses `resolveMode` entirely (never cloud, never onboarding), and it
  // hosts the namespaced engine commands (`setup`/`connect`/`update`/…).
  if (normalizedArgs[0] === "local") {
    await runLocalEntry(normalizedArgs.slice(1), env);
    return;
  }

  // `infinite app …` — the product's Desktop command. Kept ahead of the general
  // `--project` rejection so it keeps its own precise error code.
  const desktopAppCommand = extractDesktopAppArgs(normalizedArgs);
  if (desktopAppCommand) {
    if (desktopAppCommand.hasProjectSelector) {
      throw new DesktopAppClientError(
        "desktop_app_project_flag_unsupported",
        "infinite app uses Desktop's active workspace. Switch workspaces in Desktop and retry without --project."
      );
    }
  ...
  // (2) Product `help`/`version` are always allowed; product `update` explains
  // Desktop updating (the agent ships with Desktop) — NEVER the git/local-stack
  ...
  if (command === "version" || command === "--version" || command === "-v") {
    output.write(`${versionText(env)}\n`);
    return;
  }
  if (command === "update") {
    output.write(`${productUpdateText()}\n`);
    return;
  }

  // (3) Reject top-level `--project` (product turns follow Desktop's active
  // workspace) and intercept reserved local-command names BEFORE the
  // unknown-text→turn fallthrough, so `infinite sources` never becomes chat.
  ...
  const reservedNotice = reservedCommandNotice(command, normalizedArgs.slice(1));
  if (reservedNotice !== null) {
    output.write(`${reservedNotice}\n`);
    process.exitCode = 1;
    return;
  }
...
function hasTopLevelProjectFlag(args: string[]): boolean {
  return args.some((arg) => arg === "--project" || arg.startsWith("--project="));
}

// Product one-shot MESSAGE routing (§6.6 steps 4–8): probe the LIVE bridge and
...
async function runProductOneShot(dispatchArgs: string[], env: CliEnv): Promise<void> {
  ...
  // Onboarding: mac with no live bridge. Interactive callers launch + poll
  // Desktop (§6.6 step 6) and continue INTO the turn on ready; one-shot/piped
...
// `infinite local [...]` — explicit local engine, no `resolveMode`. Delegates to
// today's dispatch: a bare `infinite local` opens the local interactive session;
// `infinite local <command>` runs that command locally.
async function runLocalEntry(localArgs: string[], env: CliEnv): Promise<void> {
  const { args: dispatchArgs } = await resolveProjectFlag(localArgs, env);
  ...
  await dispatchCliCommand(command, rest, env);
}

// Shared command dispatch + render pipeline for the LOCAL lane (`infinite
// local <command>` and the explicit config-local one-shot). An unknown-command
// message falls through to LOCAL chat inside `runCliInput` — the product
// message path never comes here (it routes via `runProductOneShot`).
...
export async function runCliInput(
  ...
  if (command === "app") {
    await runDesktopAppCommand(args, env);
    return undefined;
  }
  try {
    ...
    // Unknown-command fallthrough → LOCAL chat. Only the local lane reaches
    // this (`infinite local <text>` / config-local one-shots): the product
    // entry intercepts reserved names and routes messages via the bridge in
    // `runProductOneShot` BEFORE any dispatch gets here (§6.6 step 8).
    const message = [command, ...args].join(" ");
    return chatRequest(message, env, undefined, agentRuntime, onProgress);
  }
}
...
const ACTIVE_PROJECT_EXEMPT_COMMANDS = new Set([
  ...
  "codex", "model", "recipes", "project", "app"
]);
...
function extractDesktopAppArgs(
  args: string[]
): { args: string[]; hasProjectSelector: boolean } | undefined {
  let index = 0;
  let hasProjectSelector = false;
  ...
  return args[index] === "app"
    ? { args: args.slice(index + 1), hasProjectSelector }
    : undefined;
}
...
export async function setupProjectStep(
  ...
  if (!name && existing) {
    return {
      ...
      next: "Run `infinite local project list` to see all projects."
    };
  }
  if (!name) {
    return {
      ...
      skipped: true,
      ...(options.interview ? { interview: options.interview } : {}),
      next: "Run `infinite local project new <name>` to create your first project."
    };
  }
  const createDb = options.createDb ?? createInfiniteOsDb;
...
export async function runSetupWizard(
  ...
  if (setupMode === "exit") {
    return {
      ...
      status: "skipped",
      existingInstall,
      next: "Run `infinite local setup` when you are ready to continue."
    };
  }
  const runtimeResult =
    ...
    !runtimeResult.ok && typeof runtimeResult.next === "string"
      ...
      : queryResult.ok
        ...
        : isSetupOnboardingResult(connectorsResult) && connectorsResult.next
          ...
          : "Run `infinite local setup status` to review blockers.";
  return {
    ok:
...
async function runSetupOnboardingStep(
  ...
  if (!interview) {
    return {
      ...
      status: "skipped",
      skipped: true,
      next: "Run `infinite local setup` with the onboarding interview to configure analytics providers."
    };
  }
...
async function runSetupReset(args: string[], env: CliEnv): Promise<Record<string, unknown>> {
  ...
  try {
    ...
    if (error instanceof NoActiveProjectError || error instanceof StaleActiveProjectError) {
      return {
        ...
        next: "Run `infinite local project list` then `infinite local project use <name>` first."
      };
    }
    throw error;
  }
  ...
  if (!config?.databaseUrl) {
    return {
      ...
      next: "Run `infinite local setup runtime` first."
    };
  }
  const setup = await loadSetupModule();
  const db = createInfiniteOsDb(config.databaseUrl);
  try {
    ...
    return {
      ...
      next: cleared.length > 0
        ? "Run `infinite local setup` to start a fresh setup run."
        : "No active setup runs were blocking. Run `infinite local setup status` to review setup."
    };
  } finally {
    await db.close();
  }
...
function nextFromOutstanding(
  ...
  if (outstanding.length >= 2) {
    ...
    return `${outstanding.length} providers are still paused (${labels}) — finish each handoff, then resume from the "Still paused" list above (or run \`infinite local setup resume --all\`).`;
  }
  if (outstanding.length === 1) {
    const only = outstanding[0]!;
    return `Run \`infinite local setup resume ${only.runId}\` after completing the ${formatSetupProviderLabel(only.provider)} handoff.`;
  }
  if (failedCount > 0) {
    return "Run `infinite local setup status` to review failed providers.";
  }
  return "Run `infinite local setup query` or `infinite` to continue.";
}
...
async function runLocalSetupResumeAll(input: {
  ...
  if (typeof setup.listActiveSetupRuns !== "function") {
    return {
      ...
      workflow: "onboarding",
      status: "unsupported",
      next: "This setup build does not support `resume --all`. Resume each run with `infinite local setup resume <run_id>` (see `infinite local setup status`)."
    };
  }
  ...
  if (initialOutstanding.length === 0) {
    return {
      ...
      next: "No setup runs are paused. Run `infinite local setup status` to review setup."
    };
  }
  ...
  return {
    ...
    next:
      outstanding.length > 0
        ...
        : "Run `infinite local setup status` to review setup."
  };
}
...
export function buildSetupOnboardingResult(
  ...
  if (noDataSource) {
    ok = false;
    next = "Run `infinite local setup` and select a data source (GA4 or PostHog) — nothing was configured yet.";
  } else if (outstanding.length >= 2) {
    ok = false;
    const labels = outstanding.map((entry) => formatSetupProviderLabel(entry.provider)).join(", ");
    next = `${outstanding.length} providers are still paused (${labels}) — finish each handoff, then resume from the "Still paused" list above (or run \`infinite local setup resume --all\`).`;
  } else if (outstanding.length === 1) {
    ok = false;
    const only = outstanding[0]!;
    next = `Run \`infinite local setup resume ${only.runId}\` after completing the ${formatSetupProviderLabel(only.provider)} handoff.`;
  } else if (result.failed.length > 0) {
    ok = false;
    next = "Run `infinite local setup status` to review failed providers.";
  } else {
    ok = true;
...
function buildExistingRuntimeSetupResult(env: CliEnv): Record<string, unknown> {
  ...
  return {
    ...
    next: "Run `infinite local setup runtime --reconfigure` to change runtime settings."
  };
}
  ...
  return {
    ...
    error: {
      ...
      examples: [
        ...
        "infinite local setup --provider codex --auth login",
        "infinite local setup --provider claude --auth reuse"
      ]
    }
  };
}
...
function modelSetupNeedsInputResult(): Record<string, unknown> {
  return {
    ...
    error: {
      ...
      examples: [
        "infinite local setup model",
        "infinite local model use codex gpt-5.5 --auth login",
        "infinite local model use claude claude-sonnet-4-5 --auth reuse"
      ]
    }
  };
}
...
function runtimeSetupNeedsInputResult(): Record<string, unknown> {
  return {
    ...
    error: {
      ...
      examples: [
        "infinite local setup runtime --mode local_docker",
        "infinite local setup runtime --mode external_postgres --database-url postgres://user:pass@host:5432/db",
        "infinite local setup runtime --mode supabase --database-url postgres://user:pass@host:5432/db"
      ]
    }
  };
}
    ...
    try {
      ...
      if (!found) {
        throw new StaleActiveProjectError(
          `Active project ${id} no longer exists. Run \`infinite local project list\` then \`infinite local project use <name>\`.`
        );
      }
    } finally {
      await db.close();
  ...
  if (!provider && input.isTTY === true && env.GROWTH_OS_CLI_NONINTERACTIVE !== "1") {
    ...
    if (!selected.length) {
      return {
        ...
        skipped: true,
        next: "Run `infinite local setup connectors` when you are ready to connect marketing data."
      };
    }
    const configured = [];
    ...
    return {
      ...
      section: "connectors",
      configured,
      next: "Run `infinite local setup query` or `infinite local setup status` to review readiness."
    };
  }
  const readiness = await readSetupReadiness(env);
    ...
    providers: CONNECTOR_SETUP_REGISTRY.map((entry) => ({
      ...
      setup:
        entry.provider === "google_analytics_4"
          ? "infinite local setup connectors google_analytics_4 --client-id <google_oauth_client_id>"
          : `${"infinite local setup connectors"} ${entry.provider}`
    })),
    setupReadiness: readiness,
      next:
...
export function decideConnectWizard(line: string): ConnectWizardDecision {
  ...
  if (IN_CHAT_CONNECT_OAUTH_NOTE_PROVIDERS.has(definition.provider)) {
    return {
      kind: "note",
      text: "GA4 quick-connect opens a browser — run `infinite local setup` in your terminal to connect GA4."
    };
  }
  if (IN_CHAT_CONNECT_TERMINAL_ONLY_PROVIDERS.has(definition.provider)) {
    return {
      kind: "note",
      text: `Run \`infinite local connect ${definition.provider}\` in your terminal for now.`
    };
  }
...
export async function reconnectExistingConnector(
  ...
  return {
    ...
    sourceId,
    result: response,
    next: "Reconnected. Run `infinite local setup query` or `infinite` to continue."
  };
}
  ...
  return {
    ...
    next:
      backfill?.queued === true
        ? `Meta Ads ${backfill.windowLabel} backfill queued. Run \`infinite local setup query\` or \`infinite\` to continue.`
        : initialSync?.queued === true
          ? "X timeline sync queued. Run `infinite local setup query` or `infinite` to continue."
          : "Run `infinite local setup query` or `infinite` to continue."
  };
}
...
async function runSyncCommand(args: string[], env: CliEnv, options: RunCommandOptions = {}): Promise<unknown> {
  ...
  if (!rawTarget) {
    if (!interactive) {
      throw new Error(
        "sync requires a source id, provider (e.g. meta), or 'all'. Run `infinite local sync` in a terminal to pick interactively."
      );
    }
  ...
  return {
    ...
    choices: SYNC_WINDOW_CHOICES.map((choice) => {
      ...
      return {
        ...
        cliCommand: `infinite local sync ${target} ${value}`
      };
    })
  };
}
...
function syncTargetNotFoundMessage(target: string): string {
  ...
  if (knownProvider) {
    return `No ${knownProvider} source is connected. Connect one with \`infinite local connect ${knownProvider}\` or \`infinite local setup connectors\`.`;
  }
  return `No connected source matches "${target}". Run \`infinite local sources\` to list connected sources.`;
}
...
async function runSyncWizard(
  ...
  if (!sources.length) {
    throw new Error(
      "No connected sources to sync. Connect one first with `infinite local connect <provider>` or `infinite local setup connectors`."
    );
  }
...
function connectorSetupExample(definition: ConnectorSetupDefinition): string {
  const parts = ["infinite", "local", "setup", "connectors", definition.provider];
  for (const field of definition.fields) {
  ...
  if (readiness.ready) {
    ...
    errorOutput.write(
      `App server still not responding after ${Math.round(readiness.waitedMs / 1000)}s ` +
        ...
        "check `infinite local logs app`, then retry.\n"
    );
  }
  return {
  ...
  return {
    ...
    runtime: {
      ...
      migrations: {
        ...
        logsCommand: "infinite local logs migrate"
      }
    },
    error: {
...
async function metaBudgetCommand(
  ...
  if (object !== "campaign" && object !== "adset") {
    throw new Error(`meta ${object} has no budget verb (budgets live on campaign|adset only)`);
  }
  const entityId = metaPositionalId(rest);
  if (!entityId) {
  ...
  const proceed = await metaConfirmWrite(
    "update budget for",
    object,
    `Set ${object} ${entityId} ${budgetLine}`,
    rest,
    env,
    options,
    section
...
export async function projectCommand(
      ...
      ...(standalone
        ? {
            hint:
              ...
              `Run \`infinite local project default set ${match.name}\` to make it the persisted default.`
          }
        : {})
    };
  }
  ...
  throw new Error("Usage: infinite local project <new|list|use|current|default|delete>");
}
...
async function projectDefaultCommand(args: string[], env: CliEnv): Promise<unknown> {
  ...
  throw new Error("Usage: infinite local project default <set|clear|show>");
}
  ...
  if (!readiness.ok) {
    throw new Error(
      [
        ...
        `Run \`${modelAuthRerunCommand(provider)}\`, then retry \`infinite local model use ${provider} ${model}\`.`
      ].join(" ")
    );
  }
  const selection = writeInfiniteOsModelSelection({ provider, model }, env as NodeJS.ProcessEnv);
...
function modelAuthRerunCommand(provider: InfiniteOsModelProvider): string {
  return provider === "codex"
    ? "infinite local codex login"
    : "infinite local auth login claude --mode setup-token";
}
...
function workspaceNotReadyFromSetupReadiness(readiness: SetupReadiness): Record<string, unknown> & { ok: boolean } {
  return {
    ...
    error: {
      ...
      nextSteps: [
        "infinite local setup",
        "infinite local start",
        "infinite local setup connectors",
        "infinite local model use <codex|claude> <model>",
        "infinite local auth login <codex|claude>"
      ]
    }
  };
}
      ...
      try {
        ...
        if (!row) {
          throw new NoActiveProjectError(
            `Project "${workspaceId}" was not found. Run \`infinite local project new <name>\` or pick an existing one.`
          );
        }
      } catch (error) {
...
export function renderProjectDeleteResult(result: ProjectDeleteResult): string {
  if (result.error?.code === "cannot_delete_last_project") {
    return (
      ...
      `Create another project first (\`infinite local project new <name>\`), then delete this one.`
    );
  }
  if (result.cancelled) {
...
function renderWorkspaceNotReady(result: {
  ...
  lines.push("", "Next:");
  for (const step of result.error.nextSteps ?? ["infinite local setup"]) {
    lines.push(`  ${step}`);
  }
  return lines.join("\n");
}
...
function setupSectionDetailLines(sectionId: string, result: unknown): string[] {
  ...
  if (sectionId === "connectors" && result.skipped === true) {
    return ["Skipped in quick setup. Run `infinite local setup connectors` to connect marketing data."];
  }
  if (sectionId === "runtime" && result.reused === true) {
    return ["Using existing runtime configuration. Run `infinite local setup runtime --reconfigure` to change it."];
  }
  if (sectionId === "runtime" && isRecord(result.error) && result.error.code === "growth_os_local_workspace_migration_failed") {
    ...
    lines.push("Logs: infinite local logs migrate");
    lines.push("Retry: infinite local setup or infinite local start");
    return lines;
  }
...
function renderSetupOnboardingResult(result: SetupOnboardingResult): string {
  ...
  if (noDataSource) {
    lines.push(
      ...
      "  Run `infinite local setup` and select a provider to get started."
    );
  }
    ...
    if (provider.status === "deferred") {
      ...
      lines.push(
        provider.deferralKind === "future"
          ...
          : `    Enable it: re-run \`infinite local setup\` and tick ${formatSetupProviderLabel(provider.provider)} to set it up now.`
      );
    }
    if (provider.handoff?.instructions) {
      lines.push(`    Action required: ${provider.handoff.instructions}`);
  ...
  if (result.installCommand) {
    ...
    if (notCovered.length > 0) {
      ...
      for (const provider of notCovered) {
        ...
        if (!provider.runId) {
          lines.push(`  - ${label}: not finished — run \`infinite local setup status\` to continue, then re-run the install command.`);
          ...
          lines.push(`  - ${label}: setup failed — retry with \`${setupResumeCommand({ id: provider.runId })}\` (see \`infinite local setup status\`), then re-run the install command.`);
        }
      }
    }
  }
  ...
  if (result.outstanding && result.outstanding.length > 0) {
    ...
    if (result.outstanding.length >= 2) {
      lines.push("  Or resume them all in order: `infinite local setup resume --all`");
    }
  }
...
function setupRunDetailLines(run: SetupRunSummary): string[] {
  ...
  if (isStaleSetupRunSummary(run)) {
    lines.push("⚠ This run looks stale — run `infinite local setup reset` to clear it.");
  }
  return lines;
}
